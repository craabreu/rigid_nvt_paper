...
This corresponds to the angular coefficient of such quantity \textit{vs}. time as determined by linear regression.
...
In the case of our proposed method, which we refer to here as Refined NHC, the conserved extended energy is $\refined H$, such as defined by Eq.~\eqref{eq:this extended energy}.
In Fig.~\ref{fig:energy_drift}, we present drift rates of these quantities computed in liquid water simulations carried out with different time-step sizes.
As one can observe in the inset, very large drifts occur for both methods when $\timestep \ge 6~\text{fs}$.
Up to $\timestep = 5~\text{fs}$, the long-term stability is slightly improved by our method.
Evidently, there is little or no impact of the lack of time-reversal symmetry on $R$.
Fig.~\ref{fig:energy_drift} also contains the drift in the refined Hamiltonian $\refined{\Ham{}}$ for the NVE method \cite{Silveira_2017} (circles), which almost coincides with the results of the Refined NHC scheme.
It is worth mentioning that $\timestep = 6~\text{fs}$ marks a stability threshold for the NVE method.
...
Considering a factorization scheme different to the one employed by those authors, \citeauthor{Davidchack_2010} \cite{Davidchack_2010} discovered the existence of energy flows between the two thermostats.
