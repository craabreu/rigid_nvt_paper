...
Molecular Simulation is the computacional realization of Statistical Mechanics\cite{Tuckerman2010} and requires numerical approximations in order to make possible a microsopic insight into complex physical phenomena. Under those approximations, certain Statistical Mechanics laws might not been necessarily traduced into assumptions in Molecular Simulation. For instance, it has already been shown that the unavoidable truncation errors associated with the numerical integrations of the equations of motion in Molecular Dynamics (MD) disrupt the equipartition of energy, and as a consequence the temperature becomes an undefined property\cite{Eastwood_2010}. In a previous paper,\citep{Abreu_2017} we observed the same problem now regarding the partition of the kinetic energy between translational and rotational degrees of freedom in MD of rigid bodies in the microcanonical (NVE) ensemble. That finding brough into question the method introduced by Kamberaj and co-workers\cite{Kamberaj2005}, which couples separate chains of Nos\'{e}-Hoover thermostats on the rotational and translational degrees of freedom and fails to reproduce the specified temperature for certain time step sizes.

The importance of rigid-body Molecular Dynamics stems from the fact that it is not limited to simulate molecules modelled as rigid bodies only, like the ubiquitous case of water,  but also because it is a coarse-grained strategy used for sistems designed as collections of rigid bodies like proteins and nanoparticiles\cite{Knorowski2012, Patra2013}. The approach of Kamberaj~\textit{et al.}\cite{Kamberaj2005}, which is implemented in the LAMMPS software package\cite{Plimpton1995}, has actually been used in the simulation of small molecules\cite{Aimoli_2014}, as well as in molecular motors\cite{Yan_2008,Akimov_2012,Geiger_2013}.

The Nos\'{e}-Hoover thermostat, routinely used in Molecular Dynamics, relies on the ``extended'' phase space on which ficticius variables are introduced in order to control the fluctuations of kinetic energy of the physical variables maintaining the temperature of the physical system in its specified value. The dynamics of those variables introduces non-Hamiltonian components to the equations of motion, which makes its evaluation more limited, comparing with Hamiltonian dynamics where backward error analysis is possible.

In this paper, we focus on the formulation of the equations of motion for rigid bodies in the canonical ensemble, and introduce time reversible and measure-preserving numerical integrators as well. both the translational and rotational degrees of freedom are simultaneously coupled to a unique Nos\'e-Hoover chain (NHC) thermostat. Our starting point to develop the thermostatized equations of motion begining from our previous work on the microcanonical ensemble.

In the Nos\'{e}-Hoover thermostat we will carried out a heuristic analysis comparing with results obtained with Hybrid Monte Carlo (HMC)\cite{Duane1987}, which is an exact method in the sense that the truncation erros are not present, point of reference. We also using SHAKE\cite{Ryckaert1977}, which is an iterative method commonly used in MD simulations, which employes lagrange multiplier to treat the holonomic constraints used in MD simulations. For molecules modeled as rigid structures the dynamics of rigid bodies emerges as a suitable approach. This is expected to be computationally more efficient because the constraints are treated implicitly. The property of being symplectic implies the conservation of volume in phase space. the inverse statement is not true. measure-preserving
