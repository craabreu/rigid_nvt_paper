...
\subsection{Discretization Effects on Equilibrium Thermodynamic Properties (Work in Progress)}

%In the previous section, we have shown that the extended energy drifts in NVT simulations are localized within the thermostat variables.
...
In Fig.~\ref{fig:properties}, we show the computed averages of some thermodynamic properties as a function of $\timestep$. The simulations were carried out by specifying $T = 298~\text{K}$ and the analyzed quantities correspond to temperature ($T$), potential energy ($U$) and virial ($W$) per molecule, as well as the constant-volume specific heat capacity ($C_V$).
...
They correspond to the results obtained employing $\timestep = 1~\text{fs}$ in the Refined NHC numerical scheme.
...
In addition, it should be pointed out that the temperature values in Fig.~\ref{fig:properties}(a) were computed from the simulated kinetic energies assuming equipartition, i.e. $2 \langle K \rangle = (6N -3) k_b T$.
