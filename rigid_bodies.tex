...
Finally, by observing Fig.~\ref{fig:energy_partition}(d), one can note that the refined translational and rotational kinetic energies, being estimates of their ``shadow'' counterparts, comply with Tolman's equipartition theorem \cite{Tolman_1918} for time-step sizes up to $5~\text{fs}$.
...
A significant enhancement was achieved for a system of TIP3P rigid water molecules\cite{Jorgensen_1983} by using an analytically approximation to the Shadow Hamiltonian (here referred to as \textit{refined Hamiltonian}) with a minimal computational overhead, and whose derivation was greatly facilitated by the unsplit solution of rotation introduced in our previous paper\cite{Silveira_2017}.
