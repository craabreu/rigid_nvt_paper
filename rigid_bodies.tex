...
While this is expected for real systems, discretization errors introduced by the numerical integration may distort such assumption.
Fortunately, backward error analysis allows us to identify the quantity that is actually subject to equipartition.
This is related to a shadow Hamiltonian, which coincides with the specified Hamiltonian only when the time-step size approaches zero.
...
For instance, it has been shown \cite{Eastwood_2010} that errors due to the discretization of the equations of motion can disrupt the equipartition of kinetic energy among molecules of distinct sizes.
A similar artifact occurs for the partition of kinetic energy between translational and rotational degrees of freedom in rigid-body systems \cite{Davidchack_2010, Silveira_2017}.
This brings into question the NVT-MD integrator developed by Kamberaj \textit{et al}. \cite{Kamberaj_2005} in which, by assuming translational-rotational equipartition, separate chains of Nos\'{e}-Hoover thermostats are coupled to the two classes of degrees of freedom.
In addition, as we will show here, this method fails to reproduce the specified temperature even for a time-step size $\timestep$ typically employed in MD.
...
This occurs because Nos\'{e}-Hoover chains (NHC) \cite{Martyna_1992} and other thermostat methods are based on the relation between temperature and kinetic energy that is a corollary of the equipartition theorem.
...
It seems reasonable to argue that a thermostat should regulate the shadow kinetic energy instead of the specified one.
...
In this paper, we devise a novel procedure which partially overcomes this limitation.
...
Although we restrict our detailed derivation to the case of a Nos\'{e}-Hoover chain thermostat, our new approach is applicable to other deterministic and stochastic thermostats.
It is straightforward to use it, for instance, in conjunction with global thermostats such as the Stochastic Velocity Rescaling method of Bussi \textit{et al}. \cite{Bussi_2007} and the Nos\'{e}-Hoover-Langevin thermostat of Leimkuhler \textit{et al}. \cite{Leimkuhler_2009}.

Our new method is particularly appealing for rigid-body MD, primarily because its numerical stability can extend up to time-step sizes large enough to yield pronounced discretization errors.
It should be pointed out that rigid-body MD is relevant not only because some small molecules are usually modeled as rigid structures, such as in the ubiquitous case of water \cite{Jorgensen_1983}, but also because it can provide a coarse-graining strategy for molecules that can be envisioned as collections of interconnected bodies, such as proteins and nanoparticles \cite{Miller_2002, Knorowski_2012, Patra_2013}.
...
In the case of rigid water, which served as an excellent test case for our proposal, we were able to correct discretization errors in ensemble averages computed with time-step sizes up to $5~\text{fs}$, thus outperforming not only the double-chain method of Kamberaj \textit{et al}. \cite{Kamberaj_2005}, but also a single-chain reimplementation of the method by Martyna \textit{et al}. \cite{Martyna_1996} and the stochastic rescaling method \cite{Bussi_2007} applied simultaneously to translational and angular velocities.
...
In addition, all thermostated simulations were carried out considering a time-scale constant $t_d = 100~\text{fs}$ (or $\tau = 100~\text{fs}$, in the nomenclature of Ref.~\citenum{Bussi_2007}).
...
This poses the question whether these conditions will influence the long-term stability of the method or not, especially if one tries to make use of large time steps.
...
It is defined as the angular coefficient of such quantity expressed as a function of time, determined by a least-square linear regression.
...
All simulations were carried out for $T = 298~\text{K}$ and the analyzed quantities are the estimated temperature $T_\text{est} = \frac{2\langle K \rangle}{(6N-3)k_\mini{B}}$,
...
\begin{figure*}
	...
	The ``Martyna'' scheme ($\bullet$) corresponds to our rigid-body implementation of the Nos\'{e}-Hoover chain integrator introduced in Ref.~\citenum{Martyna_1996}, with analytically computed free rotations \cite{Silveira_2017}.
	The ``Refined/NHC'' scheme ($\blacksquare$) is obtained by refining the ``Martyna'' method such as described in Sec.~\ref{sec:refined_method}.
	The ``Refined/NHC/Miller'' scheme ($\blacktriangle$) differs from the preceding one only in that an approximate solution is used for free rotations \cite{Miller_2002}.
	The ``Refined/Bussi'' scheme ($\times$) is a refined version of the stochastic method of Bussi \textit{et al}. \cite{Bussi_2007}.
	Finally, the ``Kamberaj'' scheme ($\blacktriangledown$) consists in the double-thermostat method introduced in Ref.~\citenum{Kamberaj_2005}.}
...
First, the method is applied in a conventional, refinement-free way, with the unsplit solution of Ref.~\citenum{Silveira_2017} being used for handling rotations.
Then, it is modified by introducing the refinement/reweighting procedure proposed here in Sec.~\ref{sec:refined_method}.
...
The fourth integrator we test is a refined version of the velocity-rescaling method of Bussi \textit{et al}. \cite{Bussi_2007}, which differs from the others for involving a global stochastic thermostat.
This is identified in Fig.~\ref{fig:properties} as ``Refined/Bussi''.
Finally, we also consider the double-thermostat integrator of Kamberaj \textit{et al}. \cite{Kamberaj_2005} for comparison purposes, despite its spurious energy flux described in the preceding section.
...
We stress that thermodynamic properties are estimated in distinct ways for conventional and refined integrators.
For the former, simple averages are taken from the sampled configurations.
For the latter, reweighting is carried out via Eq.~\eqref{eq:reweighting}.

In Fig.~\ref{fig:properties}(a), we see that the estimated temperature for the thermostat identified as ``Kamberaj'' decreases appreciably as the time step size increases.
This behavior is typical of Verlet-like NVE integrators\cite{Davidchack_2010, Silveira_2017}, indicating that the effect of the NHC factorization scheme of Kamberaj \textit{et al}. \cite{Kamberaj_2005} on the particle momenta is weak.
On the other hand, the stringent temperature control achieved by the thermostat named ``Martyna'' leads to a severely degraded accuracy in both $\langle U/N \rangle$ and $\langle W/N \rangle$, as noted in Figs.~\ref{fig:properties}(b) and \ref{fig:properties}(c).
