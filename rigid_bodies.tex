...
\end{tikzpicture}
\caption{Schematic representation of the executed steps in a single time-step of the refined NVT methods.
...
With the aim of highlighting the importance of reweighting, in Fig.~\ref{fig:free_energy} we show for the ``Refined/NHC'' scheme the free energy differences per water molecule ($\Delta F/N$, circles) between the sampled and target states as a function of the time-step size.
...
For this range of $\timestep$ and after reweighting, which yields Fig.~\ref{fig:energy_partition}(c), it becomes clear that our method corrects the serious disruption observed in the schemes of Martyna \textit{et al}. \cite{Martyna_1996} and Kamberaj \textit{et al}. \cite{Kamberaj_2005}.


\begin{figure*}
	\includegraphics{Figures/energy_partition.eps}
		\caption{Influence of the time-step size on the kinetic energy partition in NVT MD simulations of 903 TIP3P\cite{Jorgensen_1983} water molecules for the numerical schemes introduced by (a) Martyna \textit{et al.} \cite{Martyna_1996} and (b) Kamberaj \textit{et al.} \cite{Kamberaj_2005}, as well as for (c) our refined version of the thermostat by Martyna \textit{et al.} \cite{Martyna_1996}, for which we also consider in (d) the results obtained without applying the reweighting procedure. For each case, we depict the simulated translational (black down triangles) and rotational (black up triangles) kinetic energies, and show its deviation from the expected translational (green squares) and rotational (red x) values assuming equipartition.}
