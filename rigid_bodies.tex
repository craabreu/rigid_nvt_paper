...
To conclude this section, we discuss on the possibility of switching between alternative space-fixed frames when describing the motion of a rigid body. This will be useful when we deal with torque-free rotations in Sec.~\ref{sec:free_rotation}. Consider that a body-fixed vector $\vt d$ is, at the same instant $t$, observed as $\vt \delta = \tr{[\mt A(\vt q)]}{\vt d}$ in a space-fixed frame $\mathcal S_q$ and as $\vt \eta = \tr{[\mt A(\vt z)]}{\vt d}$ in another space-fixed frame $\mathcal S_z$. It thus follows that $\vt \eta = \tr{[\mt A(\vt z)]}{\mt A}(\vt q)\vt \delta$, which means that these two reference frames are rotated with respect to each other by a composite matrix $\mt A(\vt g) = \tr{[\mt A(\vt z)]}{\mt A}(\vt q)$. A simple relation between the unit quaternions $\vt g$, $\vt z$, and $\vt q$ is a key result in quaternion algebra.\cite{Dichmann1999} Adapted to our matrix/vector notation, such relation is
\begin{subequations}
\begin{equation}
 \label{eq:comb_rot}
 \vt g = \tr{\left[\begin{array}{cc}\vt z & \mt C(\vt z)\end{array}\right]} \vt q.
\end{equation}

The block matrix above comprises an orthogonal transformation in $\mathbb{R}^4$, which means that the inverse transformation is simply given by
\begin{equation}
\label{eq:inv_comb_rot}
\vt q = \left[\begin{array}{cc}\vt z & \mt C(\vt z)\end{array}\right] \vt g.
\end{equation}
\end{subequations}

As $\mathcal S_q$ and $\mathcal S_z$ are fixed in space, one can conclude that the quaternion $\vt g$ remains constant while both $\vt q$ and $\vt z$ vary in time as the body rotates. In addition, because Eq.~\ref{eq:relation_qdot_omega} was derived for an arbitrary space-fixed frame, it is clear that the equation $\dot{\vt z} = \tfrac{1}{2}{\vt B}(\vt z)\vt \omega$ is equally valid. Therefore, as an alternative to following the evolution of $\vt q$ along a period of time, one can select some convenient unit quaternion $\vt z$ at any instant, compute $\vt g$ using Eq.~\ref{eq:comb_rot}, and then follow the evolution of $\vt z$ from this instant on. Of course, Eq.~\ref{eq:inv_comb_rot} can be employed whenever the value of $\vt q$ is needed afterwards.
...
It is not possible to analytically integrate the equations of motion in Eq.~\ref{eq:EDO_system} for a general potential $U(\vt r,\vt q)$. Nonetheless, closed-form solutions for Eqs.~\ref{eq:EDO_q} and \ref{eq:EDO_q} are known for the particular case of torque-free rotation (i.e. when $\vt \tau = \vt 0$ at all times). Even though free rotations might seem of limited use in Molecular Dynamics at first sight, they are very useful as part of numerical integration methods based on splitting strategies. This is the topic addressed in Sec.~\ref{sec:symplectic}. As shown in Appendix~\ref{sec:derivation_euler}, Eq.~\ref{eq:EDO_pi} can be used to derive the well-known Euler equation $\mt I \dot{\vt \omega} + \vt \omega \times (\mt I \vt \omega) = \mt A \vt \tau$, which is completely decoupled from Eq.~\ref{eq:EDO_pi} when $\vt \tau = \vt 0$. It is a known fact that both the rotational energy $K_r = \frac{1}{2}\tr{\vt \omega}{\mt I}{\vt \omega}$ and the space-fixed angular momentum $\vt L = \tr{\mt A}{\mt I}{\vt \omega}$ (whose squared norm is $L^2 = \tr{\vt \omega}{\mt I}^2{\vt \omega}$) are integrals of motion in a free rotation. This allows us to analytically solve the Euler equation for the angular momentum vector $\vt \omega$ in terms of elliptic functions.\cite{Byrd_1971, Landau1976, Kosenko_1998, vanZon2007, Celledoni_2008, Thorn2012} From an initial condition $\vt \omega(0) = \tr{[\omega_1^0 \; \omega_2^0 \; \omega_3^0]}$, evaluating such solution starts by computing three prefactors $a_1$, $a_2$, and $a_3$, defined as
\begin{gather*}
a_1 = \frac{\omega_1^0}{\sqrt{1 - (\omega_2^0/\lambda_1)^2}}, \quad a_2 = \min(\lambda_1,\lambda_3), \\
a_3 = \frac{\omega_3^0}{\sqrt{1 - (\omega_2^0/\lambda_3)^2}},
\end{gather*}
where $\lambda_1 = \sqrt{\frac{L^2 - 2 I_3 K_r}{I_2(I_2 - I_3)}}$ and $\lambda_3 = \sqrt{\frac{2 I_1 K_r - L^2}{I_2(I_1 - I_2)}}$ with, as already mentioned, $I_1 > I_2 > I_3$. Note that the value of $a_2$ depends on a comparison between $\lambda_1$ and $\lambda_3$. This also occurs for $\vt \omega$, whose functional form is
\begin{subequations}
\begin{equation}
\vt \omega(t) = \left[\begin{array}{c}
a_1\text{cn}(b t + c, k^2) \\
a_2\text{sn}(b t + c, k^2) \\
a_3\text{dn}(b t + c, k^2)
\end{array}\right] \qquad \text{if} \quad \lambda_1 < \lambda_3
\end{equation}
or
\begin{equation}
\vt \omega(t) = \left[\begin{array}{c}
a_1\text{dn}(b t + c, k^2) \\
a_2\text{sn}(b t + c, k^2) \\
a_3\text{cn}(b t + c, k^2)
\end{array}\right] \qquad \text{if} \quad \lambda_1 > \lambda_3
\end{equation}
\end{subequations}
where $\text{sn}(u,m)$, $\text{cn}(u,m)$, and $\text{dn}(u,m)$ are the Jacobi elliptic functions, $k = {a_2}/{\text{max}(\lambda_1,\lambda_3)}$ is the elliptic modulus, and the constants $b$ and $c$ are given by
\begin{equation*}
b = -\frac{(I_1 - I_3)a_1 a_3}{I_2 a_2} \quad \text{and} \quad c = \text{sn}^{-1}\left(\frac{\omega_2^0}{a_2}, k^2\right).
\end{equation*}

In order to compute the inverse function $\text{sn}^{-1}$, one can use its relation with an elliptic integral in Carlson form, \cite{Carlson_1977} $\text{sn}^{-1}(x,m) = x R_F(1-x^2,1-mx^2,1)$. In this work, we employ the routines available in Ref.~\onlinecite{Galassi_2009} for the numerical evaluation of Jacobi functions and Carlson integrals.

Once we know $\vt \omega$ as a function of time, we can also solve Eq.~\ref{eq:EDO_q} analytically for the quaternion $\vt q$. Based on an approach pioneered by Kosenko\cite{Kosenko_1998} and extended by Celledoni \textit{et al}.,\cite{Celledoni_2008} we employ the factorization in Eq.\ref{eq:factorization_of_A} to accomplish such task.
...
\begin{equation*}
\Pi(x,n,m) = \text{sn}^{-1}(x,m) + \frac{1}{3}nx^3R_J(1-x^2,1-mx^2,1,1-nx^2).
\end{equation*}
