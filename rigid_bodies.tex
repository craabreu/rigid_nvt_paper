...
For instance, it has already been shown that the unavoidable errors in the discretization of the equations of motion disrupt the equipartition of kinetic energy \cite{Eastwood_2010}.
The same artifact occurs on the partition of kinetic energy between translational and rotational degrees of freedom in rigid-body Molecular Dynamics (MD) \cite{Davidchack_2009,Silveira_2017}.
This finding brings into question the NVT integrator developed by Kamberaj and co-workers\cite{Kamberaj_2005} that, assuming equipartition, couples separate chains of Nos\'{e}-Hoover thermostats on the rotational and translational degrees of freedom. In addition, this method fails to reproduce the specified temperature for time-step sizes usually employed in MD.

The relevance of rigid-body MD stems from the fact that not only some molecules are modelled as rigid bodies, like the ubiquitous case of water \cite{Jorgensen_1983}, but also it can be employed as a coarse-graining strategy for systems that might be designed as collection of interconected rigid bodies, such as proteins and nanoparticules \cite{Knorowski_2012, Patra_2013}.
As a matter of fact, the approach of \citeauthor{Kamberaj_2005} \cite{Kamberaj_2005}, which is implemented, for instance, in the software packages HOOMD-\textit{blue}\cite{Anderson_2008} and LAMMPS\cite{Plimpton_1995}, has been applied to simulate small molecules \cite{Geiger_2013, Aimoli_2014, Aimoli_2014_2}, as well as membranes\cite{Bucior_2012}, molecular motors \cite{Akimov_2012}, micelles\cite{Yan_2008} and nanoparticles \cite{Patra_2014}.

The Nos\'{e}-Hoover chain (NHC) thermostat \cite{Martyna_1992}, routinely used in MD, is based on the extended phase-space approach, in which auxiliary degrees of freedom act regulating the kinetic energy of the physical variables so as to be consistent with the Maxwell-Boltzmann distribution at a given target temperature.
To do so the NHC thermostat requires an expression relating the temperature, a macroscopic parameter, with the kinetic energy, which is a property of each microstate.
Usually, in MD the equipartition theorem plays this role. 
However, as already mentioned, this theorem is disrupted in numerical schemes with non-negligible discretization errors.
This is because numerical trajectories actually sample from the so-called \textit{shadow} Hamiltonian, which cannot be merely separated into momentum and position dependent parts.
Instead, what is usually identified as the \textit{shadow} kinetic energy contains quadratic terms in particle momenta, but also depends on particle positions\cite{Eastwood_2010}.
It seems reasonable to argue that the thermostat should control this kinetic energy, instead of the physical one.
However, the theory underlying the concept of shadow Hamiltonians correspond to backward error analysis, which is only applicable to symplectic integrators.
This is not the case of NHC numerical schemes, given that the auxiliary variables introduce non-Hamiltonian components to the equations of motion.
In this paper we introduce a procedure that partially overcomes this limitation by performing the backward error analysis on the NVE part of the numerical integrator with minimal computational overheads. 
The resulting NHC scheme, which simultaneously couples the translational and rotational degrees of freedom to a unique Nos\'{e}-Hoover chain thermostat, is formulated as a function on an approximation to the \textit{shadow} kinetic energy.
As will be shown later, this is only possible for splitting schemes in which the NVE Liouville operator is placed in the middle between the thermostat propagators. 
This new method is particularly appealing for rigid-body MD, primarily because in terms of numerical stability one is able to employ considerable time-step sizes for Verlet-type numerical schemes. However, the discretization errors begin to manifest much earlier.
The case of rigid water exemplifies this behavior and serves for an excellent case test for our method. 
In fact, we have been able to eliminate the discretization errors in numerically computed averages for time-step sizes up to 6 fs, outperforming the method introduced by \citeauthor{Kamberaj_2005} \cite{Kamberaj_2005}, as well as our implementation for rigid bodies of the numerical scheme developed by Martyna \citeauthor{Martyna_1996} \cite{Martyna_1996}.
It should be emphasize that this novel technique is not limited to rigid bodies, neither to the NHC method. \citeauthor{Bussi_2007} \cite{Bussi_2007}
The only requirement is that 

The paper proceeds as follows.
In Sec.~\ref{sec:nve} we briefly review the equations of motion in the NVE ensemble, which are the basis for developing the corresponding dynamics in the NVT ensemble.
In Sec.~\ref{sec:refined_method} we devise the corresponding numerical integrators, whose performance and correctness we assess in Sec.~\ref{sec:numerical_results}.
Finally, we present some concluding remarks in Sec.~\ref{sec:conclusion}.
%The starting point is  microcanonical (NVE) ensemble \citep{Silveira_2017} which employs the unit quaternion representation for the rotational degrees of freedom. 
\section{Methodology}
...
The action of $e^{\timestep \Liu{A}}$ is the simultaneous uniform translation (${\vt r} = {\vt r}^\ast + \timestep {\mt M}^{-1} {\vt p}^\ast$) and torque-free rotation over a full time step. In Ref.~\citenum{Silveira_2017} we derived a general solution of torque-free rotation in a form that facilitates computer implementation allowing, differently to existing schemes \cite{Kosenko_1998,vanZon2007,Celledoni_2008}, a unified treatment for both asymmetric ($I_1$ $>$ $I_2$ $>$ $I_3$) and symmetric tops (either $I_1$ = $I_2$ or $I_2$ = $I_3$).
...
On the other hand, the approximate solutions, such as the NO-SQUISH method  \cite{Miller_2002}, have proven to be very accurate in liquid-phase simulations \cite{Silveira_2017} and its formulation only involve sine and cosine terms.
...
It can be shown that the function $w(\vt x)$ that determines the invariant phase-space measure $e^{-w(\vt x)}d\vt x$ of Eq.~\ref{eq:ODE system for NVT} is given by $w = -7N \eta_1 - \sum_{j=2}^M \eta_j$.
It is worth noting that the $\eta$ coordinates are driven variables with no influence on the dynamics so, in this case, the measure-preserving condition for every individual operator can actually be relaxed. The $\eta$ coordinates are often computed for checking how well a numerical integrator conserves $H$.
...
The NHC part can be split even further as
\begin{equation*}
...
Please note that in Eq.~\ref{eq:generalized force NVT} we have employed Tolman's generalized equipartition theorem \cite{Uline_2008}, which states that
\begin{equation*}
\left\langle \tr{\vt p} \diff{\modified{\Ham{}}}{\vt p} + \tr{\vt \pi} \diff{\modified{\Ham{}}}{\vt \pi} \right\rangle = (6N - 3) k_\text{\tiny B} T.
\end{equation*}
This definition, with the \textit{refined} kinetic energy between brackets, provides a generalization for $h > 0$ of the simulated temperature \cite{Eastwood_2010}.
