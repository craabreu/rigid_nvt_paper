...
In this paper,  deals with discretization effects in a straightforward way: with a minimal computational overhead, we obtain a refined kinetic energy and a refined Hamiltonian as suitable estimators of their
shadow counterparts and then we tune the thermostatting procedure with this refined kinetic energy.
The case of rigid water has served for an excellent test case for our proposal, because its numerical stability extends up to time steps large enough to yield pronounced discretization errors.
By applying our new method to the Nos\'{e}-Hoover chain thermostat, we were able to correct the discretization errors in numerically computed averages for time-step sizes up to 5 fs.
\end{abstract}
...
Finally, a reweighting procedure provides us with ensemble averages which are, instead, consistent with the specified one. It should be emphasized that this approach is not limited to deterministic thermostats. It would be straightforward to use it, for instance, in conjunction with global stochastic thermostats such as those of Bussi \textit{et al}. \cite{Bussi_2007} and Leimkuhler \textit{et al}. \cite{Leimkuhler_2009}, for instance.
...
This allows simulations to be executed with large time steps, which could in principle save computer time.
However, as we will show now, discretization errors can become manifest much earlier than any risk of instability.
...
In Fig.~\ref{fig:properties}, we present some thermodynamic properties computed for our liquid water system using several NVT-MD methods with different time step sizes.
All simulations were carried out by specifying $T = 298~\text{K}$ and the analyzed quantities are the estimated temperature $T_\text{est} = \frac{2\langle K \rangle}{(6N-3)k_\mini{B}}$,
...
The horizontal, dotted lines are meant to help visualizing the departure of the computed averages from their expected values.
They correspond to the results obtained by employing the Refined NHC method with $\timestep = 1~\text{fs}$.
