...
%Note that momentum vectors $\vt p$ and $\vt \pi$ are scaled the same factor, which causes $\modified K$ to be scaled by the square of it, hence ${\modified K} = e^{-\alpha_1^\ast \frac{\timestep}{n}} {\modified K}^\ast$. In Sec.~\ref{sec:backward error analysis} we demonstrate why such effect on $\modified K$ is observed even though we do not use Eq.~\eqref{eq:kinetic energy} to compute it.
