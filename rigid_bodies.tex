...
\[
\label{eq:A_from_q}
\mt A(\vt q) = \left[ \begin{smallarray}{ccc}
...
Before describing the method, we consider the transformati


Method based on work of Kosenko\cite{Kosenko_1998} and Celledoni \textit{et al}.\cite{Celledoni_2008} and also related to the work of van Zon \textit{et al}.\cite{vanZon2007}.



Let $\vt L = \tr{[\mt A(\vt q)]}{\mt I}{\vt \omega}$ be the angular momentum vector 

During a torque-free rotational motion, the angular momentum vector $\vt L$ remains constant in any space-fixed frame of reference. 

In the body-fixed frame, although the 
...
\begin{equation*}
\vt v_1 = \left[\begin{smallarray}{c}
L_2 + I_2 \omega_2 \\
L_3 - I_3 \omega_3 \\
0 \\
L_1 - I_1 \omega_1
...
We can also express $\mt C = \left[\begin{smallarray}{ccc} \hat{\mt C}_1{\vt q} & \hat{\mt C}_2{\vt q} & \hat{\mt C}_3{\vt q}\end{smallarray}\right]$, where $\hat{\mt C}_1$, $\hat{\mt C}_2$, and $\hat{\mt C}_3$ are the following skew-symmetric permutation matrices:
\[
...
Recall that $\vt \pi$ is twice the quaternion-fixed version of the angular momentum vector. Thus, it is expected that Eq.~\ref{eq:euler_eq_quat} will result in the well-known Euler equation, which describes the evolution of the angular momentum in the body-fixed frame, if $\frac{1}{2} \tr{\mt B}$ is premultiplied to both sides. In order to show that this actually occurs, we start by carrying out the premultiplication to obtain
\[
\frac{1}{2} \tr{\mt B} \dot{\vt \pi} = \frac{1}{4} \tr{\mt B} \mt \Omega \vt \omega + \mt A \vt \tau.
\]

Once $\mt \Omega = \left[\begin{smallarray}{ccc} \hat{\mt B}_1{\vt \pi} & \hat{\mt B}_2{\vt \pi} & \hat{\mt B}_3{\vt \pi}\end{smallarray}\right]$, in analogy with Eq.~\ref{eq:BtBdot} we conclude that $\tr{\mt B} \mt \Omega = \mt S(\tr{\mt B}\vt \pi) = 2 \mt S(\mt I \vt \omega)$. In addition,
\[
...
\]
which is exactly the Euler equation of motion for rigid bodies.\cite{Goldstein2002} Its main advantage, in comparison with Eq.~\ref{eq:euler_eq_quat}, is that it completely decouples from the quaternion $\vt q$ when $\vt \tau = \vt 0$. In such a torque-free situation, the vector equation can be rewritten as the following system:
\begin{align*}
