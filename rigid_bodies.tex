...
\begin{abstract}
In Molecular Dynamics, sets of atoms collectively behaving as rigid bodies are often used to model entire molecules or parts thereof. Such a coarse-graining strategy eliminates degrees of freedom and allows larger time steps without abandoning the atomistic character of a model. In this paper, we rely on a particular factorization of the rotation matrix in order to express the mechanics of systems containing rigid bodies. Then, we present simplified equations of motion and devise time-reversible, measure-preserving integrators for both microcanonical (NVE) and canonical (NVT) dynamics. We also develop new expressions for computing the virial pressure of rigid- body systems under periodic boundary conditions.
\end{abstract}
...
The system in Eq.~\ref{eq:nhc_system} has a non-vanishing phase-space compressibility $\kappa = \sum_k \partial \dot{x}_k/\partial x_k$ because some phase-space variables affect their own time-derivatives. This is the case for the $3N$ momentum components in Eq.~\ref{eq:nhc_p}, the $4N$ components in Eq.~\ref{eq:nhc_pi}, and the first $M-1$ thermostat momenta in Eq.~\ref{eq:nhc_p_eta} (recalling that $\alpha_{M+1} = 0$). Thus, we have
\[
\kappa = -7N \alpha_1 - \sum_{j=1}^{M-1} \alpha_{j+1} = -7N \dot{\eta}_1 - \sum_{j=2}^M \dot{\eta}_j.
\]

Therefore, besides the conserved quantity $H$ of Eq.~\ref{eq:H_nvt}, the non-Hamiltonian system in Eq.~\ref{eq:nhc_system} has an invariant volume form in phase space given by\cite{Tuckerman_1999, Tuckerman2001, Ezra2006}
\begin{equation}
\label{eq:volume_form}
\bar{\omega} = e^{7N \eta_1 + \sum_{j=2}^M \eta_j} d{\vt r} \wedge d{\vt p} \wedge d{\vt q} \wedge d{\vt \pi} \wedge d{\vt \eta} \wedge d{\vt p_\eta}.
\end{equation}

Analysis of the probability distribution generated by the canonical equations of motion can be carried out via the generalized phase-space analysis of Tuckerman and coworkers.\cite{Tuckerman2001} For this, conservation laws must be identified and driven variables must be eliminated. For a system without external forces, the extended energy $H$ and the vector quantity $e^{\eta_1}\sum_{i=1}^N {\vt p}_i$ are integrals of motion, while the center-of-mass position can be considered as a driven variable due to the conservation of $\boldsymbol{\mathcal G}$.\cite{Tuckerman2001} Moreover, $2N$ equations must be eliminated from the system due to the constraints involving $\vt q_i$ and $\vt \pi_i$ for all $i$. By taking all these facts into account and applying the analysis of Ref.~\onlinecite{Tuckerman2001}, we can deduce that the metric determinant factor of the effective phase space is $\sqrt{g} = e^{(6N-2) \eta_1 + \sum_{j=2}^M \eta_j}$ and that the correct canonical distribution is attained if we make $L = 6N$.

\subsection{Time-Reversible and Measure-Preserving Integration of the Canonical Equations of Motion}

As in the Hamiltonian case, the evolution of a phase-space function $f(\vt r, \vt p, \vt q, \vt \pi, \vt \eta,{\vt p}_\eta)$ is given by the relation $f(t) = e^{t i\!L}f_0$, but now in the context of the generalized Liouville theorem.\cite{Tuckerman_1999, Tuckerman2001} This implies that we can employ the Trotter-Suzuki splitting to devise a time-reversible algorithm for integrating the equations of motion. In the microcanonical case, each separate term of the Liouville operator is Hamiltonian and, therefore, each factor of the splitting is a symplectic propagator. As a consequence, the whole propagator is symplectic and thus preserves the phase-space volume element. It is widely recognized that this property is important for numerical stability,\cite{Skeel1997} but it is restricted to Hamiltonian systems. For non-Hamiltonian ones, although the phase-space volume is no longer conserved, there exists an invariant measure in phase space, such as that present in the volume form shown in Eq.~\ref{eq:volume_form}. In this sense, Ezra\cite{Ezra2006} has introduced a systematic method for deriving accurate integrators which, besides being time-reversible, preserve the measure associated with the original equations of motion.

Here, we consider the usual separation of the non-Hamiltonian components of the equations of motion from the Hamiltonian ones. In this case, the generalized Liouville operator can be written as $i\!L = i\!L_\text{NHC} + i\!L_\text{NVE}$, where $i\!L_\text{NVE}$ is defined as in Eq.~\ref{eq:full_operator} and, from Eq.~\ref{eq:nhc_system}, it follows that
\begin{equation}
\label{eq:iL_NHC}
\begin{split}
...
By retaining the NVE factorization scheme defined in Eq.~\ref{eq:trotter_splitting_NVE}, we write the full exponential operator as
\begin{equation}
...
This factorization is similar to that proposed by Martyna \textit{et al}.\cite{Martyna1996} for general situations and differs from the one developed by Kamberaj \textit{et al}.\cite{Kamberaj2005} in the context of rigid bodies.........

\begin{subequations}
\begin{align}
i\!L_\text{NHC}^{(0)} &= -\alpha_1 \sum_{i=1}^N \left( \tr{\vt p}_i \diff{}{\vt p_i} + \tr{\vt \pi}_i \diff{}{\vt \pi_i}\right) + \alpha_1 \diff{}{\eta_1} \\
i\!L_\text{NHC}^{(j)} &= (G_j - \alpha_{j+1} p_{\eta_j}) \diff{}{p_{\eta_j}} + \alpha_{j+1} \diff{}{\eta_{j+1}} \\
i\!L_\text{NHC}^{(M)} &= G_M \diff{}{p_{\eta_M}}.
\end{align}
\end{subequations}

Kamberaj \textit{et al}.\cite{Kamberaj2005} proposed equations of motion and integration schemes for molecular dynamics with rigid bodies in the canonical ensemble. For this, they applied independent Nos\'e-Hoover chain\cite{Martyna1992} thermostats for the translational and rotational degrees of freedom. In addition, 

.............


Here we present a different approach. ...........
...
\begin{align*}
e^{\frac{\Delta t}{2} i\!L_\text{NHC}} = \exp \left( \frac{\Delta t}{4} G_M \diff{}{p_{\eta_M}} \right) \\
\prod_{j=M-1}^1 \exp \left\{ \frac{\Delta t}{4} \left[ (G_j - \alpha_{j+1} p_{\eta_j}) \diff{}{p_{\eta_j}} + \alpha_{j+1} \diff{}{\eta_{j+1}} \right] \right\} \\
\exp \Bigg[ -\frac{\Delta t}{2} \alpha_1 \sum_{i=1}^N \left(\tr{\vt p}_i \diff{}{\vt p_i} + \tr{\vt \pi}_i \diff{}{\vt \pi_i}\right) + \frac{\Delta t}{2} \sum_{j=1}^M \alpha_j \diff{}{\eta_j} \Bigg] \\
\prod_{j=1}^M \exp \left[\frac{\Delta t}{4} (G_j - \alpha_{j+1} p_{\eta_j}) \diff{}{p_{\eta_j}} \right].\\
\end{align*}
