...
By applying our new approach, we have been able to correct the discretization effects in numerically computed averages for time-step sizes up to 5 fs.
\end{abstract}
...
It seems then reasonable to argue that any thermostat should regulate the shadow kinetic energy instead of the specified one.
...
It is straightforward to use it, for instance, in conjunction with global stochastic thermostats such as those of Bussi \textit{et al}. \cite{Bussi_2007} and Leimkuhler \textit{et al}. \cite{Leimkuhler_2009}, for instance.
...
\subsection{Refinement of Stochastic NVT Dynamics}
\label{sec:refined_bussi}
...
Its refined version is simply obtained by substituting $K$ by $\refined{K}$ in Eq.~(A7) of Ref.~\citenum{Bussi_2007}.
...
\begin{figure}
	...
	\caption{Influence of the time-step size ($\timestep$) on the long-term drift rate ($R$) in: the extended energy ($\refined{H}$, down-pointing triangles) and its refined Hamiltonian ($\refined{\Ham{}}$, squares) part for our NVT method, as well as in the extended energy ($H$, up-pointing triangles) for the method due to Martyna \textit{et al}. \cite{Martyna_1996} and in the refined Hamiltonian ($\refined{\Ham{}}$, circles) for the numerical scheme of Eq.~\eqref{eq:splitting NVE} with unsplit solution of rotation.}
...
Up to $h = 6~\text{fs}$, these rates practically coincide with those obtained for $\refined H$ with the Refined NHC scheme.
This indicates that the major causes for the observed drift took place during evaluations of the NVE propagator.
...
Note that no drift occurs for $\refined{\Ham{}}$, not even for the largest considered time steps, which shows that the thermostat stabilizes the numerical integration of the physical variables.
...
Particularly, one can see in Fig.~\ref{fig:num_stab}(a) that, for each considered time-step size, the extended energy $H$ drifts upwards until reaching a steady-state value at which the integrator becomes stable.
...
%Nevertheless, this finding did not result in a revision of the method of Kamberaj \textit{et al}. \cite{Kamberaj_2005} in the literature.
...
\begin{figure}
	...
	\caption{Long-term drift rate in (a) the extended energy and (b) each thermostat energy for the NVT method introduced by Kamberaj \textit{et al}. \cite{Kamberaj_2005}. In part (b), the negative values correspond to the thermostat coupled to the translational degrees of freedom.}
	\label{fig:num_stab}
\end{figure}

\subsection{Correction of Discretization Effects on Thermodynamic Properties}

In the previous section, we have seen that drifts in the extended energy of an NVT simulation can accumulate in the thermostat-related variables, thus preventing full-fledged instability from happening.
This allows simulations to be executed with large time steps, which could in principle be used to save computer time.
However, as we will show in this section, discretization errors can become manifest much earlier than does any risk of instability.
Fortunately, our proposed methodology is capable of mitigating the effect of these errors on the ensemble averages computed for the simulated system.

In Fig.~\ref{fig:properties}, we present some thermodynamic properties computed for the liquid water system using several NVT-MD methods with different time step sizes.
All simulations were carried out by specifying $T = 298.0~\text{K}$ and the analyzed quantities are the estimated temperature $T_\text{est} = \frac{2\langle K \rangle}{(6N-3)k_\mini{B}}$,
...
The horizontal, dotted lines are meant to help visualizing the departure of the computed ensemble averages from their expected values.
They correspond to average results obtained with $\timestep = 1~\text{fs}$ by employing the three refined methods.
Error bars represent 95\% confidence intervals obtained using the Overlapping Batch Means estimator \cite{Meketon_1984, Flegal_2010} and standard uncertainty propagation formulas.
...
Fig.~\ref{fig:properties} contains results obtained by using five different NVT integrators.
Our measure-preserving extension to rigid bodies of the splitting scheme developed by Martyna \textit{et al}. \cite{Martyna_1996} is the basis of three of them.
...
First, the method is applied in a conventional (that is, refinement-free) way using the unsplit solution of Ref.~\citenum{Silveira_2017} for handling rotations.
...
Next, we substitute the unsplit rotation by the simpler NO-SQUISH splitting of Miller \textit{et al}. \cite{Miller_2002}, denoting the obtained integrator by ``Refined/Miller'' in Fig.~\ref{fig:properties}.
We note that our refinement procedure is rigorously sustained (see Appendix \ref{sec:rigid body shadow hamiltonian}) for unsplit rotations only.
However, NO-SQUISH rotations have been shown to approach them almost indistinguishably in the case of water if small time steps are employed \cite{Silveira_2017}.
The fourth integrator we test here is a refined version of the velocity-rescaling method of Bussi \textit{et al}. \cite{Bussi_2007}, which differs from the other ones for involving a single stochastic (rather than deterministic) thermostat.
...
Before discussing the results shown in Fig.~\ref{fig:properties}, we stress that the thermodynamic properties are estimated by simple averages taken from the sampled configurations in the case of a conventional method (either Martyna or Kamberaj), whereas reweighting via Eq.~\eqref{eq:reweighting} is carried out in the case of a refined one.

%Despite the non-equilibrium process occurring within the thermostats, it is worth showing the results obtained with the method of Kamberaj \textit{et al}. \cite{Kamberaj_2005}, because that is not the only problematic issue.
...
As a result, the potential energy and virial are not severely influenced by $\timestep$ as they are in the scheme of Martyna \textit{et al}. \cite{Martyna_1996}. Figs.~\ref{fig:properties}(a) and \ref{fig:properties}(b) clearly show that the temperature control described in Sec.~\ref{sec:nvt} leads to a severely degraded accuracy in $\langle U \rangle$, which systematically increases with $\timestep$.
As demonstrated in Fig.~\ref{fig:nve}, the discretization errors are non-negligible even for a small step size of $2~\text{fs}$.
Thus, the results for the method of Martyna \textit{et al}. \cite{Martyna_1996} can be interpreted as follows: if the kinetic energy is restricted to adopt a certain value, in accordance with the target temperature, the unavoidable discretization errors will accumulate in the remaining degrees of freedom.
...
Note that a small, still clearly influencing systematic error is present, as the computed averages are slightly modified for $\timestep > 5~\text{fs}$.
...
SAY THAT BUSSI AND MARTYNA, BOTH WITHOUT REFINEMENT, YIELD SIMILAR RESULTS.
...
As expected, the free energy is only negligible for $h = 1~\text{fs}$.
...
For this range of $\timestep$ and after reweighting, which yields Fig.~\ref{fig:energy_partition}(b), it becomes clear that our method corrects the serious disruption observed in the schemes of Martyna \textit{et al}. \cite{Martyna_1996} and  Kamberaj \textit{et al}. \cite{Kamberaj_2005}.
...
Another helpful identity taken from Appendix B of Ref.~\citenum{Silveira_2017} is ${\mt B}(\vt q){\vt \omega} = ( \sum_{k=1}^3 \omega_k \hat{\mt B}_k ) \vt q$, where each $\hat{\mt B}_k$ is a skew-symmetric permutation matrix (i.e. $\tr{\hat{\mt B}}_k = -\hat{\mt B}_k$).
