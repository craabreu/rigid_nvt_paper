...
This brings into question the NVT-MD method developed by Kamberaj, Low, and Neal \cite{Kamberaj_2005} (KLN) who, by assuming equipartition, coupled two independent thermostats to the translational and rotational degrees of freedom.
...
Debenedetti.... \cite{Palmer_2018} \cite{Limmer_2011, Limmer_2013}
...
\begin{figure*}
	...
	Finally, the KLN scheme ($\blacktriangledown$) consists in the double-thermostat method of Kamberaj, Low, and Neal \cite{Kamberaj_2005}.}
...
The fourth integrator we test is a refined version of the velocity rescaling method of Bussi, Donadio, and Parrinello \cite{Bussi_2007}, which differs from the others for involving a global stochastic thermostat.
...
Fig.~\ref{fig:properties}(d) contains results for the constant-volume specific heat capacity.
Interestingly, these results do not get affected by the discretization errors when $\timestep \leq 5~\mathrm{fs}$, not even if the KLN method is employed.
Given that the specific heat of water changes little with temperature in the vicinity of $298~\mathrm{K}$, it is not surprising to observe that $c_\mini{V}$ remains unaltered while other properties vary.
...
In this paper, we have devised a general approach that integrates backward error analysis into Nos\'{e}-Hoover chains and other global thermostats.
...
In practice, we employ backward error analysis to compute, with minimal computational overhead, a refined version of the kinetic energy, which serves as an estimate for its shadow analogue.
This turns out to be crucial for simulations that display considerable discretization errors, in which a clearer definition of the simulation temperature \cite{Eastwood_2010} is obtained by applying Tolman's generalized equipartition theorem to the refined kinetic energy rather than to the conventional one.

A significant enhancement was achieved for a system of rigid TIP3P-water molecules \cite{Jorgensen_1983}.
...
Particularly, we have been able to remove serious artifacts observed when using the method of Kamberaj, Low, and Neal \cite{Kamberaj_2005} or a rigid-body extension of the standard Nos\'{e}-Hoover chain thermostat \cite{Martyna_1992, Martyna_1996}.
The refinement/reweighting approach described in Sec.~\ref{sec:refined_method} corrected discretization effects in computed thermodynamic properties of water for time-step sizes up to $5~\text{fs}$.
Without the refinement, we would not recommend the use of time steps larger than $1~\text{fs}$.
Having overcome such barrier is of paramount importance as it allows one to actually exploit the high numerical stability and useful coarse-graining provided by rigid-body MD.

Finally, it is worth remarking that our method is not limited to rigid bodies.
It can be employed, for instance, in conjunction with other constrained MD schemes, such as SHAKE \cite{Ryckaert_1977} and RATTLE \cite{Andersen_1983}, for instance.
Of course, restraints arising from these methods must be properly taken into account, as done here for the time-derivatives of orientation quaternions.
