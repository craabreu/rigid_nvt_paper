...
\newcommand{\timestep}{h}
\newcommand{\shadow}[1]{\widetilde{#1}}
...
\end{align}
\end{subequations}
where $k_\text{\tiny B}$ is the Boltzmann constant, $T$ is the system temperature, and $L = 6N-3$ if the vector-sum of all linear momenta is set as null, as usual (otherwise, $L = 6N$).\cite{Martyna_1994} As recommended in Ref.~\citenum{Martyna_1992}, one can make $Q_1 = L k_\text{\tiny B} T t_d^2$ and $Q_j = k_\text{\tiny B} T t_d^2$ for $j \geq 2$, where $t_d$ is a characteristic time scale of the system.
%Finally, $\shadow K$ is the total kinetic energy of the system, whose computation (not using Eq.~\ref{eq:kinetic energy}, hence the distinct symbol) is the central feature of the method we propose.
...
H_\text{\tiny NVT} = \Ham{} + {\textstyle\sum\limits_{j=1}^{M}} \frac{p_{\eta_j}^2}{2Q_j} + L k_\text{\tiny B} T\eta_1 + k_\text{\tiny B} T {\textstyle\sum\limits_{j=2}^{M}} \eta_j.
\end{equation}
...
%Note that momentum vectors $\vt p$ and $\vt \pi$ are scaled the same factor, which causes $\shadow K$ to be scaled by the square of it, hence ${\shadow K} = e^{-\alpha_1^\ast \frac{\timestep}{n}} {\shadow K}^\ast$. In Sec.~\ref{sec:backward error analysis} we demonstrate why such effect on $\shadow K$ is observed even though we do not use Eq.~\eqref{eq:kinetic energy} to compute it.
Finally, propagators $e^{\frac{\timestep}{4n} \mathcal{L}_1}$ to $e^{\frac{\timestep}{4n} \mathcal{L}_M}$ are applied again, but now in the increasing order of their indices. It is worth noting that the $\eta$'s are driven variables with no influence on the dynamics, but they are often computed just for checking how well the integrator conserves $H_\text{\tiny NVT}$.

The whole procedure described above is repeated $n$ times. Because of its low computational cost, the compensation required for increasing the time step $\timestep$ without degrading the accuracy of the NHC part (that is, employing a larger $n$) causes a small impact in the overall integration effort. In contrast, evaluating the NVE part is expensive for involving force/torque computations and its accuracy goes down very quickly as $\timestep$ increases \cite{Davidchack_2010, Silveira_2017}. A high-order integration scheme\cite{Omelyan_2007, vanZon_2008} could possibly admit larger time steps, but it would raise both cost and complexity because for entailing the computation (or numerical estimation) of force/torque gradients. The processed splitting approach \cite{Omelyan_2008} seems promising, but its extension to the NVT case without doing back-and-forth processing at every time step would require further theoretical development.

Here we take a simpler path. Instead of attempting to improve the evaluation of $e^{\timestep \Liu{\tiny NVE}}$, we try to quantify the obtained deviations and then use them to tune the thermostatting procedure and properly weight the sampled configurations when canonical averages are computed.

A known property of splitting methods applied to Hamiltonian ODE systems is that they provide approximate solutions which are also Hamiltonian. Thus, a trajectory generated to be roughly consistent with the specified Hamiltonian $\Ham{}$ will be exactly consistent with a nearby, albeit unknown, function $\shadow{\mathcal H}$. It is also known that $\shadow{\mathcal H}$, referred to as a shadow Hamiltonian, depends on the time step size $\timestep$ and that, for a time-reversible splitting like Eq.~\eqref{eq:splitting NVE}, $\shadow{\mathcal H} = \Ham{} + \mathcal{O}(\timestep^2)$. Estimating the trailing term of this equality, either analytically or numerically, is a task of backward error analysis. As demonstrated in Sec.~\ref{sec:backward error analysis}, the shadow Hamiltonian for a Verlet-type method applied to a system of rigid bodies can be better approximated as $\shadow{\Ham{}} = \shadow K + \shadow U + \mathcal{O}(h^4)$, where $\shadow K$ and $\shadow U$ will be called shadow kinetic and shadow potential energies, respectively. The former is given by
\begin{equation}
\label{eq:shadow kinetic energy}
\shadow K = \frac{1}{2} \tr{ \left[\begin{array}{c} \vt p \\ \vt \pi \end{array}\right]} \shadow{\mathbf \Omega}(\vt r, \vt q, \timestep^2) \left[\begin{array}{c} \vt p \\ \vt \pi \end{array}\right],
\end{equation}
where $\shadow{\mathbf \Omega}$ is a complicated matrix-valued function of $\vt r$, $\vt q$, and $\timestep^2$, but independent of both $\vt p$ and $\vt \pi$. The shadow potential is also independent of these momenta and is given by
\begin{equation}
\label{eq:shadow potential energy}
\shadow U = U - \frac{\timestep^2}{24} \left[ \tr{\vt F} {\mt M}^{-1} {\vt F} + \tr{\vt \tau} \tr{\mt A}(\vt q) {\mt I}^{-1} {\mt A}(\vt q) {\vt \tau} \right].
\end{equation}

Therefore, the outcome of solving Eq.~\eqref{eq:ODE system for NVE} using a Trotter-Suzuki splitting will be close to the exact solution of Eq.~\eqref{eq:ODE system for NVE}, but it will be even closer to the solution of a system composed of equations
\begin{equation}
\label{eq:shadow ODE system}
\left[\begin{array}{c} \dot{\vt r} \\ \dot{\vt q} \end{array}\right] =
\left[\begin{array}{c} \diff{\shadow K}{\vt p} \\ \diff{\shadow K}{\vt \pi} \end{array}\right] =
\shadow{\mathbf \Omega}(\vt r, \vt q, \timestep^2) \left[\begin{array}{c} \vt p \\ \vt \pi \end{array}\right],
\end{equation}
as well as $\dot{\vt p} = -\diff{\shadow{\mathcal H}}{\vt r}$ and $\dot{\vt \pi} = -\diff{\shadow{\mathcal H}}{\vt q}$. In our proposed method, we acknowledge this fact and thus use the Nos\'e-Hoover chain to provide a canonical distribution consistent with $\shadow{\mathcal H}$ (rather than $\Ham{}$) at temperature $T$. Note that this entails solving Eq.~\eqref{eq:ODE system for NVT} exactly as described before, with the only requirement of replacing $K$ by $\shadow K$ in the definition of the generalized force $G_1$. By contrasting Eqs.~\eqref{eq:generalized force NVT}, \eqref{eq:shadow kinetic energy}, and \eqref{eq:shadow ODE system}, we can see that
\begin{equation}
G_1 = 2 \shadow K - L k_\text{\tiny B} T.
\end{equation}

Direct evaluation of Eq.~\eqref{eq:shadow kinetic energy} for computing $\shadow K$ is quite complex and expensive. However, one can do it very easily using the approach devised by \citeauthor{Eastwood_2010} \cite{Eastwood_2010}, which consists in using numerical differentiation to estimate the derivatives in the left-hand side of Eq.~\eqref{eq:shadow ODE system}. Denoting the obtained estimators as $\dot{\vt r}_\text{\tiny NVE}$ and $\dot{\vt q}_\text{\tiny NVE}$ and substituting Eq.~\eqref{eq:shadow ODE system} into Eq.~\eqref{eq:shadow kinetic energy}, we find out that
\begin{equation}
\shadow K = \frac{1}{2} \left( \tr{\vt p} \dot{\vt r}_\text{\tiny NVE} + \tr{\vt \pi} \dot{\vt q}_\text{\tiny NVE} \right).
\end{equation}

- Pratical estimation of numerical derivatives: virtual steps and interpolation


- Scaling of $\shadow K$ during the NHC integration.

Reweighting:
\begin{equation}
\label{eq:reweighting}
\langle A \rangle = \frac{\sum\limits_{i=1}^{N_s} A(\vt x_i) \exp \left[{\frac{\shadow{\Ham{}}(\vt x_i) - {\Ham{}}(\vt x_i)}{k_\text{\tiny B} T}}\right]}{\sum\limits_{i=1}^{N_s} \exp \left[{\frac{\shadow{\Ham{}}(\vt x_i) - {\Ham{}}(\vt x_i)}{k_\text{\tiny B} T}}\right] }.
\end{equation}
...
When describing the motion of a rigid body, the translational and rotational modes can be decoupled and expressed, respectively, as the evolution of a center-of-mass coordinate $\vt r(t)$ in a space-fixed frame of reference and the evolution of a quaternion $\vt q(t) = \tr{[\begin{array}{cccc} q_1 & q_2 & q_3 & q_4 \end{array}]}$ which continuously satisfies the unit-norm constraint $\tr{\vt q}{\vt q} = 1$ \cite{Goldstein_2002}. In this way, the position of any constituent particle $j$ will vary with time as $\vt R_j(t) = \vt r(t) + \tr{\mt A}(\vt q(t))\vt d_j$, where $\vt d_j$ is the (constant) position of particle $j$ in a body-fixed frame of reference centered in $\vt r$ and ${\mt A}(\vt q)$ is the body's orientation matrix (that is, the rotation matrix connecting the body-fixed and space-fixed frames) expressed as a function of the quaternion components. The superscript $t$ stands for transposition, which is equivalent to inversion in the case of a canonical transformation like $\mt A$ \cite{Goldstein_2002}. The central feature explored in Ref.~\citenum{Silveira_2017} is the fact that $\mt A$ admits the simple factorization
\begin{equation}
\label{eq:factorization_of_A}
{\mt A}(\vt q) = \tr{\mt B}(\vt q) {\mt C}(\vt q),
\end{equation}
where
\begin{equation*}
\label{eq:def_B_and_C}
...
The intermediate frame is exactly the subspace of $\mathbb{R}^4$ that must contain the rate-of-change vector $\dot{\vt q}$ if the norm of $\vt q$ is to remain constant, since $\tr{\vt q} {\vt q} = 1$ implies that $\tr{\vt q} \dot{\vt q} = 0$. In the Hamiltonian approach for the motion of a rigid body, the momentum $\vt \pi$ conjugated to $\vt q$ is given by $\vt \pi = 2 \mt B(\vt q) \mt I \vt \omega$, where $\vt \omega$ is the three-dimensional angular velocity and $\mt I$ is a diagonal matrix containing the body's principal moments of inertia.\cite{Silveira_2017} This means that $\vt \pi$ is the quaternion-fixed representation of the angular momentum $\vt L = \mt I \vt \omega$, thus being orthogonal to $\vt q$ as well. After this notation has been introduced, the Hamiltonian of a rigid body can be written as
\begin{equation}
\label{eq:Hamiltonian}
\Ham{} = K(\vt p, \vt q, \vt \pi) + U(\vt r,\vt q),
\end{equation}
where $K$ and $U$ are the kinetic and potential energies of the body, respectively. While the potential energy can have any functional form, the kinetic energy is given by
\begin{equation}
\label{eq:kinetic energy}
K = \frac{1}{2} \tr{\vt p} {\mt M}^{-1} {\vt p} + \frac{1}{8} \tr{\vt \pi} {\mt B}(\vt q) {\mt I}^{-1} \tr{\mt B}(\vt q) {\vt \pi},
\end{equation}
where $\vt p$ is the linear momentum of the body and $\mt M$ is a diagonal matrix containing the body's mass in all three diagonal entries. Note that the kinetic energy is composed of two independent contributions, a translational and a rotational one. An advantage of this notation is that Eqs.~\eqref{eq:Hamiltonian} and \eqref{eq:kinetic energy} can also represent a system of $N$ interacting rigid bodies once we consider that vectors $\vt r$, $\vt p$, $\vt q$, and $\vt \pi$ contain the respective properties of all bodies piled together. The same occurs for matrices $\mt M$ and $\mt I$, which turn into diagonal matrices of size $3N \times 3N$. In addition, $\mt B(\vt q)$ becomes a block-diagonal matrix with $4N$ rows and $3N$ columns.

The Liouville operator \cite{Tuckerman_2010} that stems from the Hamiltonian in Eq.~\eqref{eq:Hamiltonian} is\cite{Silveira_2017}
\begin{align*}
\label{eq:Lioville operator}
\Liu{\tiny NVE} &= \tr{\vt p} {\mt M}^{-1} \diff{}{\vt r} + \frac{1}{2} \tr{\vt \omega} \tr{\mt B}(\vt q) \diff{}{\vt q} \\
&+ \tr{\vt F} \diff{}{\vt p} + \left[ \frac{1}{2} \tr{\vt \omega} \tr{\mt B}(\vt \pi) + 2 \tr{\vt \tau} \tr{\mt C}(\vt q) \right] \diff{}{\vt \pi},
\end{align*}
where $\vt F$ and $\vt \tau$ are, respectively, the resultant force and resultant torque exerted on the body (or on $N$ bodies), both represented in the space-fixed frame of reference, whereas $\vt \omega$ can be obtained by
\begin{equation*}
\vt \omega = \frac{1}{2} {\mt I}^{-1} \tr{\mt B}(\vt q) {\vt \pi}.
\end{equation*}

The time evolution of any phase-space vector $\vt x$ is governed by $\tr{\dot{\vt x}} = \Liu{\tiny NVE} \tr{\vt x}$ \cite{Tuckerman_2010}. Therefore, the equations of motion that describe the rigid-body motion are \cite{Silveira_2017}
\begin{subequations}
	\label{eq:EDO_system}
	\begin{align}
	&\dot{\vt r} =
...
The solution for a system like $\tr{\dot{\vt x}} = \Liu{\tiny NVE} \tr{\vt x}$ with initial condition $\vt x(0) = \vt x_0$ can be formally expressed as $\tr{\vt x}(t) = e^{t \Liu{\tiny NVE}} \tr{\vt x}_0$. Although the exact effect of the exponential operator %$e^{t \Liu{\tiny NVE}}$
cannot be determined for an arbitrary potential $U(\vt r, \vt q)$, a symplectic numerical solution can be achieved by a Verlet-type integration. For two non-commutative Liouville operators $\Liu A$ and $\Liu B$ and a real constant $\timestep$, the symmetric Baker-Campbell-Hausdorff (BCH) formula can be expressed as \cite{Hairer_2006}
\begin{align}
...
\end{align}
where $[X,Y]$ is the commutator of operators $X$ and $Y$, defined as $[X,Y] = XY - YX$. The remaining terms of the infinite series involve ever-deepening, nested commutators \cite{Hairer_2006}. With $\Liu{\tiny NVE} = \Liu{A} + \Liu{B}$ and a small $\timestep$ value, we can approximate the BCH equation as
\begin{equation*}
e^{\timestep \Liu{\tiny NVE}} = e^{\frac{\timestep}{2} \Liu{B}} e^{\timestep \Liu{A}} e^{\frac{\timestep}{2} \Liu{B}} + \mathcal{O}(\timestep^3),
\end{equation*}
which is known as the Trotter-Suzuki splitting formula \cite{Trotter_1959, Suzuki_1976}. In the rigid-body case, by making
\begin{subequations}
...
\end{equation}
\end{subequations}
it becomes possible to determine exactly the individual actions of operators $e^{\timestep \Liu{A}}$ and $e^{\frac{\timestep}{2} \Liu{B}}$, with $\timestep$ being a small time step $\Delta t$. The latter operator promotes a kick in the linear and quaternion momenta given by ${\vt p} \leftarrow {\vt p} + \frac{\timestep}{2} {\vt F}$ and ${\vt \pi} \leftarrow {\vt \pi} + \timestep {\mt C}({\vt q}) {\vt \tau}$. The former, in turn, performs a simultaneous force-free translation and torque-free rotation during the length of a time step.
%
%
%first one is the solution of
%\begin{subequations}
%	\label{eq:EDO translation and rotation}
%	\begin{align}
%	&\dot{\vt r} =
%	%+\diff{\Ham{}}{\vt p} =
%	{\mt M}^{-1} {\vt p}, \\
...
%	&\dot{\vt \pi} =
%	%-\diff{\Ham{}}{\vt q} =
%	\frac{1}{2} \mt B(\vt \pi) \vt \omega, \label{eq:EDO_pi}
%	\end{align}
%\end{subequations}
%
% and $N_s$ is the number of steps. We emphasize that each exponential operator above will act on (that is, have as initial condition) the result of the operator at its right-hand side.
%
%
%
%for this system of equations e rigid body motion which preserves the symplectic structure of Eq.~\eqref{eq:EDO_system}. For a sum of two operators $i\!L_A$ and $i\!L_B$, such formula isThe can be used to devise 
%The full integrator is second-order in $\timestep$ because the leading-order term of the error at every step is proportional to $\timestep^3$ and, in addition, $N_s = t/\timestep$.\cite{Tuckerman_2010} The splitting of the Liouville operator of Eq.~\eqref{eq:full_operator} we employ in this paper is
%\begin{equation}
%\label{eq:trotter_exact_rotation}
%e^{\timestep i\!L} = e^{\frac{\timestep}{2} i\!L_b} e^{\timestep i\!L_{t+r}} e^{\frac{\timestep}{2} i\!L_b} + \mathcal{O}(\timestep^3).
%\end{equation}
%
%This is the Velocity Verlet algorithm\cite{Tuckerman_2010} with center-of-mass translations and rigid-body rotations taking place simultaneously. Update of forces and torques is required only after the step of translation \& rotation has been completed. Because the solution of torque-free rotation involves a single step in the numerical solver, the scheme represented by Eq.~\ref{eq:trotter_exact_rotation} will be referred to as the unsplit rotation integrator. We remark that van Zon and co-workers\cite{vanZon2007PRE, vanZon2008} have already employed this algorithm using the formulation proposed in Ref.~\citenum{vanZon2007}. Other splitting schemes with higher-order approximations have also been used for rigid bodies.\cite{Omelyan2007, Omelyan2008, vanZon2008} However, they result in methods that are more complex and often require expensive force-gradient calculations. Moreover, it is unclear how their benefits would extend to non-Hamiltonian numerical integrators, such as those often employed for molecular dynamics in ensembles other than the microcanonical one.
%
%
%
%
...
%\end{split}
%\]
%Following Miller \textit{et al}.,\cite{Miller_2002} each complete rotation is factorized as
%\begin{align*}
%\label{eq:splitting_rot}
%e^{\timestep i\!L_r} =  e^{\frac{\timestep}{2} i\!L_3} e^{\frac{\timestep}{2} i\!L_2} e^{\timestep i\!L_1} e^{\frac{\timestep}{2} i\!L_2} e^{\frac{\timestep}{2} i\!L_3},
%\end{align*}
%where ${\timestep}$ is the time step size. The splitting of the Liouville operator of Eq.~\eqref{eq:full_operator} we employ in this paper is
%\begin{equation}
%\label{eq:trotter_splitting_NVE}
%e^{\timestep i\!L_\text{NVE}} = e^{\frac{\timestep}{2} i\!L_b} e^{\timestep i\!L_r} e^{\timestep i\!L_t} e^{\frac{\timestep}{2} i\!L_b}.
%\end{equation}

\subsection{Backward Error Analysis}
\label{sec:backward error analysis}

Let us now represent the operators $\Liu A$ and $\Liu B$ using Poisson brackets involving Hamiltonians $\Ham A$ and $\Ham B$, that is, $\Liu A = \{\circ,\Ham A\}$ and $\Liu B = \{\circ,\Ham B\}$. By employing the properties of anti-commutativity and Jacobi identity \cite{Hairer_2006}, we can deduce that
\begin{align*}
...
This means that the commutator $[\Liu A,\Liu B]$ is, in fact, a new Liouville operator $\Liu C$ whose associated Hamiltonian is $\Ham C = {\Liu A}{\Ham B}$. By applying this procedure recursively, we can rewrite the right-hand side of Eq.~\eqref{eq:symmetric BCH} as $e^{\timestep \Liu S}$, where $\Liu S$ is the Liouville operator whose associated Hamiltonian is
\begin{equation*}
\label{eq:general shadow hamiltonian}
\shadow{\Ham{}} = \Ham A + \Ham B + \frac{h^2}{24} (2 \Liu A + \Liu B){\Liu A}{\Ham B} + \mathcal{O}(h^4).
\end{equation*}

Therefore, a splitting method meant to approximately reproduce the dynamics of a system with Hamiltonian $\Ham{} = \Ham A + \Ham B$ will, in fact, reproduce exactly (putting round-off issues aside) the dynamics of another system with Hamiltonian $\shadow{\Ham{}}$ as above. In the literature, $\shadow{\Ham{}}$ is usually referred to as a shadow Hamiltonian.

The first new result we present here is an analytical form for the shadow Hamiltonian concerning a system of rigid bodies whose dynamics is integrated via the unsplit rotation method of Ref.~\citenum{Silveira_2017}. A detailed derivation is provided in Appendix~\ref{sec:rigid body shadow hamiltonian} and we summarize the final results as follows. First of all, we rewrite the total kinetic energy of the original system, Eq.~\eqref{eq:kinetic energy}, as a quadratic form
\begin{equation*}
...
With $\nabla^2 U$ being the Hessian matrix of the potential energy with respect to the entries of $\vt r$ and $\vt q$, we define a shadow kinetic energy $\shadow K$ as
\begin{equation*}
\shadow K = \frac{1}{2} \tr{ \left[\begin{array}{c} \vt p \\ \vt \pi \end{array}\right]} \shadow{\mathbf \Omega}(\vt r, \vt q) \left[\begin{array}{c} \vt p \\ \vt \pi \end{array}\right],
\end{equation*}
where
\begin{equation*}
\shadow{\mathbf \Omega} = {\mt \Omega} + \frac{\timestep^2}{6} {\mt \Omega} (\nabla^2 U) \tr{\mt \Omega}.
\end{equation*}

Although $\shadow{\mathbf \Omega}$ is still symmetric, it is no longer a block-diagonal matrix like $\mt \Omega$, due to the inter-body interactions accounted for in $U(\vt r, \vt q)$ and to the dependency of forces and torques on both positions and orientations. As a result, the shadow kinetic energy cannot, in principle, be split into independent translational and rotational contributions.

In addition to $\shadow K$, we also define a shadow potential energy $\shadow U$ as
\begin{equation*}
\shadow U = U - \frac{\timestep^2}{24} \left( \tr{\vt F} {\mt M}^{-1} {\vt F} + \tr{\vt \tau} \tr{\mt A} {\mt I}^{-1} {\mt A} {\vt \tau} \right).
\end{equation*}

Finally, the shadow Hamiltonian for a systems of rigid body becomes
\begin{equation}
\shadow{\Ham{}} = \shadow K + \shadow U + \mathcal{O}(h^4).
\end{equation}

\subsection{Generalized Equipartition}

The important thing is that $\shadow{\mathbf \Omega}$ depends exclusively on the positions and orientations, which makes $\left[\begin{array}{c} \diff{\shadow K}{\vt p} \\ \diff{\shadow K}{\vt \pi} \end{array}\right] = \shadow{\mathbf \Omega} \left[\begin{array}{c} \vt p \\ \vt \pi \end{array}\right]$. In this way, the equipartition theorem (valid at least in this direction? something to do with integrating through momenta at least internally to the integral in coordinates?) states that
\begin{equation*}
...
e^{\timestep i\!L} = e^{\frac{\timestep}{2} i\!L_\text{NHC}} e^{\frac{\timestep}{2} i\!L_b} e^{\timestep i\!L_r} e^{\timestep i\!L_t}  e^{\frac{\timestep}{2} i\!L_b} e^{\frac{\timestep}{2} i\!L_\text{NHC}}.
\end{equation}
...
\end{align}
\end{subequations}
with $j$ varying from $1$ to $M-1$ in Eq.~\eqref{eq:iL_NHC_j}. Then, each propagator $e^{\frac{\timestep}{2} i\!L_\text{NHC}}$ present in Eq.~\eqref{eq:iL_NHC} is factorized as
\[
e^{\frac{\timestep}{2} i\!L_\text{NHC}} = \Bigg[ \prod_{j=M}^1 e^{\frac{\timestep}{4} i\!L_\text{NHC}^{(j)} } \Bigg] e^{\frac{\timestep}{2} i\!L_\text{NHC}^{(0)} } \Bigg[ \prod_{j=1}^M e^{\frac{\timestep}{4} i\!L_\text{NHC}^{(j)} } \Bigg].
\]
...
\end{align*}
where $\phi(x) = (1-e^{-x})/x$. We might note that, in order to avoid numerical issues with $x \rightarrow 0$, we evaluate $\phi(x)$ as $\sum_{n=0}^3 {(-1)^n x^n}/{(n+1)!}$ whenever $|x| \leq 10^{-4}$. The solution above is equivalent to that found in Ref.~\citenum{Martyna_1996}.
...
In the new propagator, $e^{\frac{\timestep}{2} i\!L_\text{NHC}^{(0)} }$ is removed:
\[
e^{\frac{\timestep}{2} i\!L_\text{NHC}^\ast} = \Bigg[ \prod_{j=M}^2 e^{\frac{\timestep}{4} i\!L_\text{NHC}^{(j)} } \Bigg] e^{\frac{\timestep}{2} i\!L_\text{NHC}^{(1)} } \Bigg[ \prod_{j=2}^M e^{\frac{\timestep}{4} i\!L_\text{NHC}^{(j)} } \Bigg].
\]
...
\begin{split}
e^{\timestep i\!L} = e^{\frac{\timestep}{2} i\!L_\text{NHC}^\ast} e^{\frac{\timestep}{2} i\!L_b^\ast} e^{\timestep i\!L_r} e^{\timestep i\!L_t}  e^{\frac{\timestep}{2} i\!L_b^\ast} e^{\frac{\timestep}{2} i\!L_\text{NHC}^\ast}.
\end{split}
\end{equation}
...
In this subsection we briefly review a simple method that serves for checking the consistency between the sampled and theoretical energy distributions. The procedures, implemented by Shirts,\cite{Shirts_2013} rely on quantitative statistical analysis of fitting, and it is possible to perform linear and nonlinear fitting or a maximum likelihood approach too. First, recall that in the NVT ensemble the probability of observing an energy E is given by
\[
...
In this section, we analyze the numerical integrators introduced in Subsection~\ref{sec:numerical_solvers}, and compare the results with those obtained employing the method of Kamberaj~\textit{et al.}\cite{Kamberaj_2005}, which is implemented in the LAMMPS\cite{Plimpton_1995} software package. For that, we considered a system of 900 TIP3P\cite{Jorgensen_1983} water molecules in a cubic box of length {\AA} subject to periodic boundary conditions. The intermolecular interactions were truncated at 9 \AA and to avoid a discontinuity in the force, we applied the so-called shifted-force (SF) method\cite{Allen_1989, Fennell_2006, Toxvaerd_2011} to treat both the Coulombic and the 12-6 Lennard-Jones (LJ) interactions.

It is not the purpose of this work to propose an optimal factorization scheme for the NVT dynamics, but rather to prove that a unique thermostat chain is capable of maintaining the average temperature at its specified value while providing the proper sampling of the canonical ensemble. First we present some results as first mented. That is, the proposed methods coupling to a single thermostat, while in Kamberaj the translational and rotational degrees of freedom are separatedly coupled. In Table~\ref{table:new_results}, it can be seen that the integrators proposed here, which are represented by the Eqs.~\eqref{eq:trotter_splitting_NHC} and ~\eqref{eq:modified_splitting}, systemcatically achive the specifed temperature, regardless of the time step size. By contrast, the numerical solver introduced by Kamberaj and co-workers\cite{Kamberaj_2005} fails to yield the specified temperature for even the smaller time step size of 1 fs.
...
\begin{figure}
	...
	\caption{Linear plot of the log ratio probabilities with the true slope (solid line) and the measured slope from the linear regression analysis (symbols). Simulation of liquid water using Eq.~\eqref{eq:trotter_splitting_NHC}, $\timestep$ = 1~fs, $\bar{T}_1$ = 296.03~K, $\bar{T}_2$ = 304.99~K.}
	\label{fig:checkensemble}
\end{figure}

\begin{figure}
	\includegraphics{maxwell-botlzamm-paper}
	\caption{Probability distribution of the kinetic energy with sampled values and those calculated with Eq.~\eqref{eq:mb}. Simulation of liquid water using Eq.~\eqref{eq:trotter_splitting_NHC}, $\timestep$ = 1~fs, $\bar{T}$ = 304.99~K.}
\label{fig:mbdistribution}
\end{figure}

Finally, in Table~\ref{table:ensemblevalidation} we present the results of the maximum likelihood approach applied for both NVT integration schemes. It can be seen that the deviations from the true slope, $\timestep$ = 8.970~K, are less than $1\sigma$. This means that effectively, the sampled values do not deviate from the correct distribution to a statistically noticeable level.
...
%  \captionof{figure}{Histograms of pressure obtained in this work (filled bars) and using LAMMPS %(unfilled bars). Simulation of liquid water with NVE integration. $\timestep : 1 fs$  }
%\end{center}
...
\section{Shadow Hamiltonian of Rigid Bodies}
...
If a splitting is done so that $\Ham A = K(\vt p, \vt q, \vt \pi)$ and $\Ham B = U(\vt r, \vt q)$, then from Eq.~\eqref{eq:general Lioville operator} we have that $\Liu{A} = \tr{K_{\vt p}}\diff{}{\vt r} + \tr{K_{\vt \pi}}\diff{}{\vt q} - \tr{K_{\vt q}}\diff{}{\vt \pi}$ and $\Liu{B} = -\tr{U_{\vt r}}\diff{}{\vt p} - \tr{U_{\vt q}}\diff{}{\vt \pi}$. Therefore, $\Liu A \Ham B = \tr{K_{\vt p}} U_{\vt r} + \tr{K_{\vt \pi}} U_{\vt q}$. This makes
\begin{align*}
...
\end{align*}
where the fact that $\tr{U_{\vt q \vt r}} = U_{\vt r \vt q}$ has been exploited. As reported in Ref.~\citenum{Silveira_2017}, the identity $\tr{\mt B}(\vt q) {\vt \pi} = -\tr{\mt B}(\vt \pi) {\vt q}$ is useful for performing differentiations. The first-order derivatives of $K$ with respect to $\vt p$, $\vt q$, and $\vt \pi$ are, respectively,
\begin{align*}
&K_{\vt p} = {\mt M}^{-1} {\vt p}, \\
...
Pre-multiplication by $\tr{K_{\vt q}}$ will introduce a product $\tr{\mt B}(\vt \pi) {\mt B}(\vt q)$, which has been shown in Ref.~\citenum{Silveira_2017} to be equal to ${\mt S}\left( \tr{\mt B}(\vt \pi) {\vt q} \right) = -2 {\mt S}({\mt I} {\vt \omega})$, where the operator ${\mt S}(\cdot)$ builds a skew-symmetric matrix from the entries of a vector. In addition, post-multiplication by $U_{\vt q}$ will introduce a product $\tr{\mt B}(\vt q) U_{\vt q}$, which is equal to $-2 {\vt \tau}^b$, where ${\vt \tau}^b$ is the body-fixed frame representation of the torque. Then, the fact that ${\mt S}(\vt x) {\vt y} = -{\mt S}(\vt y) {\vt x}$ leads to
\begin{equation*}
\tr{K_{\vt q}} K_{\vt \pi \vt \pi} U_{\vt q} = \frac{1}{2} \tr{\vt \omega} {\mt S}({\mt I}^{-1} {\vt \tau}^b) {\mt I} {\vt \omega}.
\end{equation*}

Another helpful identity taken from Appendix B of Ref.~\citenum{Silveira_2017} is ${\mt B}(\vt q){\vt \omega} = ( \sum_{k=1}^3 \omega_k \hat{\mt B}_k ) \vt q$, where each $\hat{\mt B}_k$ is a skew-symmetric permutation matrix (i.e. $\tr{\hat{\mt B}}_k = -\hat{\mt B}_k$). This allows us to obtain
\begin{align*}
...
The second term in the right-hand side of the equation above is identically null whenever (a quadratic form whose matrix is skew-symmetric), which makes $\tr{K_{\vt q}} K_{\vt \pi \vt \pi} U_{\vt q} = -\tr{K_{\vt \pi}} K_{\vt \pi \vt q} U_{\vt q}$. Finally,
\begin{align*}
