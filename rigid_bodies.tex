...
Recall that the symmetric scheme of \citeauthor{Martyna_1996} \cite{Martyna_1996} is supposed to preserve the extended energy $H$ given by Eq.~\eqref{eq:nvt extended energy}.
...
This has already been noticed by \citeauthor{Davidchack_2010} \cite{Davidchack_2010} and also by \citeauthor{Bond_2007} \cite{Bond_2007}, who in this respect stated that ``the thermostat acts as a sort of reservoir for numerical errors''.

To end this section, we report in Fig.~\ref{fig:num_stab} an anomalous behavior observed when the method of \citeauthor{Kamberaj_2005} \cite{Kamberaj_2005} is employed.
...
As already mentioned, the scheme due to \citeauthor{Kamberaj_2005} \cite{Kamberaj_2005} couples separate NHC thermostats to the rotational and translational degrees of freedom.
Considering a different factorization scheme, \citeauthor{Davidchack_2010} \cite{Davidchack_2010} discovered the existence of energy flows between the two thermostats.
%Nevertheless, this finding did not result in a revision of the method of \citeauthor{Kamberaj_2005} \cite{Kamberaj_2005} in the literature.
This is confirmed for the method of \citeauthor{Kamberaj_2005} \cite{Kamberaj_2005} in Fig.~\ref{fig:num_stab}(b), where we depict separately the contribution of each thermostat to the extended energy $H$.
...
\begin{figure}
	...
	\caption{Long-term drift rate in (a) the extended energy and (b) each thermostat energy for the NVT method introduced by \citeauthor{Kamberaj_2005} \cite{Kamberaj_2005}. In part (b), the negative values correspond to the thermostat coupled to the translational degrees of freedom.}
	\label{fig:num_stab}
\end{figure}
...
Despite the non-equilibrium process occurring within the thermostats, it is worth showing the results obtained with the method of \citeauthor{Kamberaj_2005} \cite{Kamberaj_2005}, because that is not the only problematic issue.
