...
\]
where $\vt F_j$ is the force applied on particle $j$, expressed in the space-fixed frame of reference, and thus $\vt F_i$ is the resultant force on the body. As demonstrated in \citeauthor{Silveira_2017} \cite{Silveira_2017}, $\partial{U}/\partial \vt q_i$ is given by
\[
...
\subsection{Shadow Hamiltonian}
\label{sec:shadown hamiltonian}

First-order derivative of a scalar function with respect to a vector variable (gradient vector):
\begin{equation*}
f_{\vt x} = \diff{f}{\vt x}
\end{equation*}

Second-order derivative of a scalar function with respect to two vector variables (Hessian matrix):
\begin{equation*}
f_{\vt x \vt y} = \diff{(\tr{f}_{\vt x})}{\vt y}.
\end{equation*}

Liouville operator:
\begin{equation}
\label{eq:general Lioville operator}
\Liu{} = \tr{{\mathcal H}_{\vt p}} \diff{}{\vt r} - \tr{{\mathcal H}_{\vt r}} \diff{}{\vt p} = \{\circ,\Ham{}\}.
\end{equation}

For non-commutative operators $A$ and $B$ and a real constant $h$, the symmetric Baker-Campbell-Hausdorff (BCH) formula (Ref.~\citenum{Hairer_2006}, p.~86) can be expressed as
\begin{equation}
\label{eq:symmetric BCH exponential}
e^{\frac{h}{2} B} e^{h A} e^{\frac{h}{2} B} = e^{h S},
\end{equation}
with
\begin{equation}
\label{eq:symmetric BCH operator}
S = A + B + \frac{h^2}{24} \left[2 A + B,[A,B]\right] + \mathcal{O}(h^4).
\end{equation}

In this expression, $[A,B]$ is the commutator of operators $A$ and $B$, defined as $[A,B] = AB - BA$. The remaining terms of the infinite series involve nested commutators with increasing levels of recursion \cite{Hairer_2006}. If $A$ and $B$ are Liouville operators $\Liu A = \{\circ,\Ham A\}$ and $\Liu B = \{\circ,\Ham B\}$, respectively, then we have that
\begin{align*}
...
In the expression above, the Jacobi identity \cite{Hairer_2006} has been employed after some convenient permutations. This result means that the commutator of two Liouville operators is, in fact, a new Liouville operator $\Liu C$ with Hamiltonian $\Ham C = {\Liu A}{\Ham B}$. By induction, we can conclude that the operator $S$ in Eq.~\eqref{eq:symmetric BCH operator} is also a Liouville operator $\Liu S$ with Hamiltonian
\begin{equation}
\label{eq:symmetric BCH hamiltonian}
\Ham S = \Ham A + \Ham B + \frac{h^2}{24} (2 \Liu A + \Liu B){\Liu A}{\Ham B} + \mathcal{O}(h^4).
\end{equation}

Considering a separable Hamiltonian $\Ham{}(\vt r, \vt p) = K(\vt p) + U(\vt r)$, where $K$ and $U$ are the kinetic and potential energies, respectively, the Velocity Verlet integrator consists in splitting $\Ham{} = \Ham A + \Ham B$, where $\Ham A = K(\vt p)$ and $\Ham B = U(\vt r)$. Therefore, $\Liu A = \tr{K_{\vt p}} \diff{}{\vt r}$ and $\Liu B = -\tr{U_{\vt r}} \diff{}{\vt p}$, which makes ${\Liu A}{\Ham B} = \tr{K_{\vt p}} U_{\vt r} = \tr{U_{\vt r}} K_{\vt p}$. Hence, Eq.~\eqref{eq:symmetric BCH hamiltonian} becomes
\begin{equation*}
\Ham S = K + U + \frac{{\Delta t}^2}{24} \left(2\tr{K_{\vt p}} U_{\vt r \vt r} K_{\vt p} - \tr{U_{\vt r}} K_{\vt p \vt p} U_{\vt r} \right) + \mathcal{O}({\Delta t}^4).
%\Ham S = K + U + \frac{h^2}{12} \tr{\left(\diff{K}{\vt p}\right)}\left(\sdiff{U}{\vt r}\right) \left(\diff{K}{\vt p}\right) + \frac{h^2}{24} \tr{\left(\diff{U}{\vt r}\right)}\left(\sdiff{K}{\vt p}\right) \left(\diff{U}{\vt r}\right) + \mathcal{O}(h^4).
\end{equation*}

The kinetic energy is given by $K = \frac{1}{2} \tr{\vt p}{\mt M}^{-1}{\vt p}$, where $\mt M$ is a diagonal matrix containing the mass associated to each degree of freedom. This makes $K_{\vt p} = {\mt M}^{-1}{\vt p}$ and $K_{\vt p \vt p} = {\mt M}^{-1}$. The vector containing the force acting on each degree of freedom is $\vt F = -U_{\vt r}$. Thus,
\begin{align*}
\Ham S &= K + U + \frac{{\Delta t}^2}{24} \left(2\tr{\vt p} {\mt M}^{-1} U_{\vt r \vt r} {\mt M}^{-1} {\vt p} - \tr{\vt F} {\mt M}^{-1} {\vt F} \right) \\ &+ \mathcal{O}({\Delta t}^4).
\end{align*}

Let us introduce $\tilde{\mt M}$ as a mass tensor\cite{Bennett_1975} whose definition is given by
\begin{equation*}
\tilde{\mt M}^{-1} = {\mt M}^{-1} + \frac{{\Delta t}^2}{6} {\mt M}^{-1} U_{\vt r \vt r} {\mt M}^{-1}.
\end{equation*}

Therefore,
\begin{equation}
\Ham S = \frac{1}{2}\tr{\vt p} \tilde{\mt M}^{-1} {\vt p} + {\tilde U} + \mathcal{O}({\Delta t}^4),
\end{equation}
where
\begin{equation*}
{\tilde U} = U - \frac{{\Delta t}^2}{24}\tr{\vt F}{\mt M}^{-1} {\vt F}.
\end{equation*}

The important thing is that $\tilde{\mt M}$ depends exclusively on the coordinates, which makes $\diff{\tilde K}{\vt p} = \tilde{\mt M}^{-1} {\vt p}$. In this way, the equipartition theorem (valid at least in this direction? something to do with integrating through momenta at least internally to the integral in coordinates?) states that
\begin{equation*}
...
\hline
\multicolumn{8}{c}{Single-Thermostat Analogue of the Method of \citeauthor{Kamberaj2005} \cite{Kamberaj2005}} \\
...
Overall:
\begin{equation*}
\Ham S = \frac{1}{2} \tr{\left[\begin{array}{c} \vt p \\ \vt \pi \end{array}\right]} \tilde{\mt \Omega} \left[\begin{array}{c} \vt p \\ \vt \pi \end{array}\right] + \tilde{U} + \mathcal{O}({\Delta t}^4),
\end{equation*}
where
\begin{equation*}
\tilde{U} = U - \frac{\Delta t^2}{24} \left( \tr{\vt F} {\mt M}^{-1} {\vt F} + \tr{\vt \tau}_b {\mt I}^{-1} {\vt \tau}_b \right)
\end{equation*}
and
\begin{equation*}
\tilde{\mt \Omega} = {\mt \Omega} + \frac{\Delta t^2}{6} \left( {\mt \Omega} {\mt U}_{\vt x \vt x} \tr{\mt \Omega} + {\mt \Gamma} \right)
\end{equation*}
with
\begin{equation*}
