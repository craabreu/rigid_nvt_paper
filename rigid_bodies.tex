...
Prompted by a particular factorization of the rotational matrix, in a previous paper\cite{Abreu_2017} we derived symplectic numerical integrators in the NVE ensemble: one that uses the exact solution for torque-free rotation and another approach, based on the method introduced by Miller~\textit{et al.},\cite{Miller2002}, that employs an approximate solution for rotation. For mathematical details the reader is referred to our paper.\cite{Abreu_2017} Next, we restrict ourselves to present the expressions required to develop the NVT case.
...
The matrix $\mt A$ which can be computed from a given unit quaternion $\vt q$ as explained in Refs.~\onlinecite{Allen1989,Miller2002} admits the following factorization:
\begin{equation}
