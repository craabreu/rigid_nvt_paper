...
\end{equation*}
whose denominator is the norm of both $\vt u$ and $\vt v$. The problem has thus been reduced to determining how the coefficients $b$ and $c$ vary with time. Further reduction is achieved by realizing that both the unit-norm constraint ($\tr{\vt z}{\vt z} = 1$) and the orthogonality condition ($\tr{\vt u}{\vt v} = 0$) hold at all times, which makes
\begin{equation*}
\tr{\vt z}{\vt z} = \frac{b^2 \tr{\vt u}{\vt u} + 2bc\tr{\vt u}{\vt v} + c^2 \tr{\vt v}{\vt v}}{2L[L-I_1\omega_1(t)]} = b^2 + c^2 = 1.
\end{equation*}

Therefore, the values of $b$ and $c$ are restricted to a unit circle, so that we can finally rewrite $\vt z(t)$ as
\begin{equation*}
\vt z(t) = \frac{\vt u_1(t) \cos \theta(t) + \vt u_2(t) \sin \theta(t)}{\sqrt{2L[L-I_1\omega_1(t)]}}.
\end{equation*}

$\dot{\theta} = \frac{2K_r - L\omega_1(t)}{2[L - I_1\omega_1(t)]}$

\begin{equation*}
\theta = \frac{L}{2I_1}(t - t_0) + \frac{2 I_1 K_r - L^2}{2 L I_1\omega_p} \int_{t_0}^t \frac{\omega_p}{1 - \alpha [\omega_1(s)/a_1]}ds,
\end{equation*}
where $\alpha = I_1a_1/L$.

From Ref.~\onlinecite{Byrd_1971}, we deduce that
\begin{gather*}
\int \tfrac{1 - \alpha^2}{1 - \alpha \cn u}du = \Pi(\sn u,-\eta,k^2) + \frac{\alpha\arctan\left(\frac{\sn u\sqrt{k^2+\eta}}{\dn u}\right)}{\sqrt{k^2+\eta}}
\end{gather*}

\begin{gather*}
\int \tfrac{1 - \alpha^2}{1 - \alpha \dn u}du = \Pi(\sn u,-k^2\eta,k^2)
+ \frac{\alpha\arctan\left(\frac{\sn u\sqrt{1 + k^2\eta}}{\cn u}\right)}{\sqrt{1 + k^2\eta}}
\end{gather*}

Finally, by defining
\begin{equation*}
\eta = \frac{(\alpha {a_2}/{\lambda_1})^2}{1-\alpha^2}
\quad \text{and} \quad
\beta = \sqrt{\eta + ({a_2}/{\lambda_3})^2}
\end{equation*}

We have
\begin{equation*}
\theta = \frac{L}{2I_1\omega_p}(\omega_p t + \epsilon) + \frac{2 I_1 K_r - L^2}{2LI_1\omega_p(1-\alpha^2)} \int_0^t \frac{(1-\alpha^2)\omega_p}{1 - \alpha [\omega_1(\tau)/a_1]}d\tau,
\end{equation*}

\begin{equation*}
\theta = \frac{L}{2I_1} t + \frac{2 I_1 K_r - L^2}{2LI_1\omega_p(1-\alpha^2)} [F(\omega_p t + \epsilon) - F(\epsilon)],
\end{equation*}


\begin{equation*}
\theta = \frac{L}{2I_1}t + \frac{2 I_1 K_r - L^2}{2LI_1\omega_p(1-\alpha^2)} \left[\omega_p t + h(t) - h(0)\right],
\end{equation*}
where
\begin{equation*}
h = \Theta\left({\omega_2}/{a_2},-\eta,k^2\right) + \frac{\alpha}{\beta} \arctan\left( \beta \frac{\omega_2/a_2}{\omega_3/a_3} \right)
\end{equation*}

\subsection{Integrals}

\begin{gather*}
C(u) = \int_0^u \frac{1 - \alpha^2}{1 - \alpha \cn x}dx = \\
C(u) = u + \text{sgn}(\cn u) \Theta(\sn u,\eta) + 2\Theta(1,\eta) \left\lfloor \frac{u}{2 K} \right\rceil + \\
+ \frac{\alpha}{\sqrt{k^2+\eta}} \arctan\left(\frac{\sqrt{k^2+\eta} \; \sn u}{\dn u}\right)
\end{gather*}

\begin{gather*}
D(u) = \int_0^u \frac{1 - \alpha^2}{1 - \alpha \dn x}dx = \\
F_\dn(u) = u + \text{sgn}(\cn u) \Theta(\sn u,k^2\eta) + 2\Theta(1,k^2\eta) \left\lfloor\frac{u}{2 K}\right\rceil + \\
+ \frac{\alpha}{\sqrt{1 + k^2\eta}} \left[\arctan\left(\frac{\sqrt{1 + k^2\eta} \; \sn u}{\cn u}\right) + \pi \left\lfloor \frac{u}{2 K} \right\rceil\right]
\end{gather*}


\begin{align*}
\Theta(x,n,m) &= \Pi(x,-n,m) - F(x,m) = \\
&= -\frac{1}{3}nx^3R_J(1-x^2,1-mx^2,1,1+nx^2).
\end{align*}

\subsection{DRAFT}

As these basis vectors are known functions of time, the problem of determining $\vt z(t)$ reduces to finding out how the two coefficients of the combination vary with time. Examination of $\vt v_1$ and $\vt v_2$ above makes it clear that these coefficients coincide with the entries $z_3$ and $z_4$ of $\vt z$, in such a way that
\begin{equation}
