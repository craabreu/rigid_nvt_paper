...
Deterministic and stochastic Molecular Dynamics methods aimed at reproducing a target temperature share the equipartition theorem as a premise.
While this intends to mimic the real world physics, discretization errors, which are intrinsic to numerical integrators, may severely distort such assumption.
Fortunately, Backward Error Analysis is a powerful tool that allows to identify the quantity that is actually subject to equipartition, which is a shadow Hamiltonian that only coincides with the specified Hamiltonian
when the time-step size approaches zero.
...
By applying our new method to the Nos\'{e}-Hoover chain thermostat, we were able to correct the discretization errors in numerically computed averages for time-step sizes up to 6 fs.
\end{abstract}
...
\subsection{Refined Hamiltonian in NVE Simulations}
...
As described in Sec.~\ref{sec:refined_method}, the refined kinetic energy $\refined K$ can be explicitly included in an NVT integrator without a substantial increase in computational effort.
...
This poses the question whether these conditions will influence the long-term stability of the method, especially if one tries to employ long time steps.

Here we define a drift rate, denoted by $R$, as the long-term rate of change of a quantity in an equilibrated MD simulation.
This corresponds to the angular coefficient of such quantity \textit{vs}. time and is determined by linear regression.
...
As one can observe, long-term stability is slightly improved by our method for $\timestep \le 5~\text{fs}$, indicating that there is little or no impact of the lack of time-reversal symmetry on $R$.

Fig.~\ref{fig:energy_drift} also contains drift rates in the refined Hamiltonian $\refined{\Ham{}}$ for the NVE method \cite{Silveira_2017}, which almost coincide with the results for the Refined NHC scheme up to $h = 6~\text{fs}$.
However, the NVE integration becomes fully unstable for time step sizes above this value.
Each NVT integrator, on the other hand, manages to remain numerically stable at least up to $h = 10~\text{fs}$, but displays an exceedingly large drift in its corresponding extended energy, as one can see the inset of Fig.~\ref{fig:energy_drift}.
...
Note that no drift occurs for $\refined{\Ham{}}$, not even for the largest considered time-step size, which shows that the thermostat stabilizes the numerical integration of the physical variables.
...
To end this section, we report in Fig.~\ref{fig:num_stab} an anomalous behavior encountered when the method of \citeauthor{Kamberaj_2005} \cite{Kamberaj_2005} is employed.
