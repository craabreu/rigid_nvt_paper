...
It is not possible Eq.~\ref{eq:EDO_system}
...
Considering that $I_1 > I_2 > I_3$, the prefactors of the angular momenta $\omega_1$, $\omega_2$, and $\omega_3$ are, respectively,
\begin{align}
\mt A = \left[\begin{array}{ccc}
\frac{\omega_1^0}{\sqrt{1 - (\omega_2^0/\lambda_1)^2}} & 0 & 0 \\
0 & \min(\lambda_1,\lambda_3) & 0 \\
0 & 0 & \frac{\omega_3^0}{\sqrt{1 - (\omega_2^0/\lambda_3)^2}}
\end{array}\right], \\
\end{align}
\begin{align}
A_1 = \frac{\omega_1^0}{\sqrt{1 - (\omega_2^0/\lambda_1)^2}}, \\
A_2 = \min(\lambda_1,\lambda_3), \; \text{and} \\
...
A_1\text{cn}(a t + b,k) \quad \text{if} \quad \lambda_1 < \lambda_3 \\
A_1\text{dn}(a t + b,k) \quad \text{if} \quad \lambda_1 > \lambda_3
\end{cases} \\
\omega_2(t) &= A_2 \text{sn}(a t + b,k) \\
\omega_3(t) &= \begin{cases}
A_3\text{dn}(a t + b,k) \quad \text{if} \quad \lambda_1 < \lambda_3 \\
A_3\text{cn}(a t + b,k) \quad \text{if} \quad \lambda_1 > \lambda_3
\end{cases}
\end{align}
\end{subequations}
where
\begin{align*}
&a = -\frac{(I_1-I_3)A_1A_3}{I_2 A_2},\\
&b = \text{sn}^{-1}\left(\frac{\omega_{2,0}}{A_2},k\right),\quad\text{and}\\
&k = \frac{A_2}{\text{max}(\lambda_1,\lambda_3)}
\end{align*}
