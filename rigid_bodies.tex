...
This finding brings into question the NVT integrator developed by Kamberaj and co-workers\cite{Kamberaj_2005} which, assuming equipartition, couples separate chains of Nos\'{e}-Hoover thermostats on the rotational and translational degrees of freedom. In addition, as will be shown later, this method fails to reproduce the specified temperature for time-step sizes usually employed in MD.

The relevance of rigid-body MD stems from the fact that not only some molecules are usually modelled as rigid bodies, like the ubiquitous case of water \cite{Jorgensen_1983}, but also it can be employed as a coarse-graining strategy for systems that might be designed as collection of interconected rigid bodies, such as proteins and nanoparticules \cite{Knorowski_2012, Patra_2013}.
...
This requires an expression relating the temperature, a macroscopic parameter, with the kinetic energy, which is a property of each microstate.
...
Instead, what is usually identified as the \textit{shadow} kinetic energy contains quadratic terms in particle momenta accompanied by particle positions dependency \cite{Eastwood_2010}.
It seems then reasonable to argue that the thermostat should control this kinetic energy, instead of the physical one.
However, the theory underlying the concept of shadow Hamiltonian correspond to backward error analysis, which is only applicable to symplectic integrators.
...
The resulting NHC scheme, which simultaneously couples the translational and rotational degrees of freedom to a unique Nos\'{e}-Hoover chain thermostat, controls the fluctuations of an approximation to the \textit{shadow} kinetic energy.
...
In the following, we analyze the performance of the NVT scheme proposed in  this paper and compare the results with those obtained employing the measure-preserving version of the integrator due to \citeauthor{Martyna_1996} \cite{Martyna_1996} as well as the method of \citeauthor{Kamberaj_2005} \cite{Kamberaj_2005}
...
Before presenting our main results, we first examine the departure of the \textit{refined} Hamiltonian from the specified one, as this is the most direct way to determine the importance of the discretization erros \cite{Engle_2005}. This is essential for the discussion of the NVT case. 

In Fig.~\ref{fig:nve} we show the time evolution of both $\modified{\Ham{}}$ (dotted lines) and $\Ham{}$ (solid lines) for the water system undergoing NVE dynamics with different time-step sizes ($\timestep$). For this, we used the numerical scheme of Eq.~\ref{eq:splitting NVE} with the unsplit solution of rotation. As expected, $\modified{\Ham{}}$ is very well conserved and close to $\Ham{}$ for $\timestep$ = 1 fs, but fairly rapidly separates from $\Ham{}$ when increasing $\timestep$. 
...
Considering a factorization scheme different to the one employed by those authors, Davidchack \cite{Davidchack_2009} discovered the existence of energy flows from one thermostat to the other.
...
Fig. clearly shows that a temperature control as described in Sec.~\ref{sec:nvt} leads to a severely degraded accuracy in $\langle U \rangle$, which systematically increases with $\timestep$.
