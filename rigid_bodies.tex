...
\end{equation}
where $\refined{\mathbf \Omega}$ is a matrix-valued function of $\vt r$, $\vt q$, and $\timestep^2$ given by $\tilde{\mt \Omega} = {\mt \Omega} + ({\timestep^2}/{6}) {\mt \Omega} {\mt U}_{\vt x \vt x} \tr{\mt \Omega}$.
In this definition, ${\mt U}_{\vt x \vt x}$ is the Hessian matrix of the potential energy with respect to $\vt r$ and $\vt q$, while ${\mt \Omega}$ is a block-diagonal matrix defined so that $K = \frac{1}{2} \tr{[\substack{\vt p \\ \vt \pi}]} {\mt \Omega} [\substack{\vt p \\ \vt \pi}]$.
...
\end{equation*}
where ${\mt \Pi}(\vt q)$ is a block-diagonal projection matrix, with each block given by ${\mt \Pi}_i = {\mt 1} - {\vt q}_i \tr{\vt q}_i$, where $\mt 1$ is the identity matrix.
...
Moreover, as such dependency lies exclusively in $\refined K$, an outcome of this theorem for a system with $6N-3$ degrees of freedom is that
\begin{equation*}
\left\langle \tr{\vt p} \diff{\refined K}{\vt p} + \tr{\vt \pi} \diff{\refined K}{\vt \pi} \right\rangle = (6N - 3) k_\text{\tiny B} T,
\end{equation*}
which becomes $2\langle \refined{K} \rangle = (6N - 3) k_\text{\tiny B} T$ if the refined kinetic energy is given by Eq.~\eqref{eq:modified kinetic energy}.
...
\node at (0.0,-0.2)[below]{$r_{t-2h}$};
\node at (2.2,-0.2)[below]{$r_{t-h}$};
\node at (4.4,-0.2)[below]{$r_{t}$};
\node at (6.6,-0.2)[below]{$r_{t+h}$};
