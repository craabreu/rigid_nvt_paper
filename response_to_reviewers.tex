...
\textbf{Answer}:
Our (unfulfilled) intention here was to suggest that the effect caused by numerical artifact is similar to that associated with small temperature variations in experiments, where the average energy changes while $c_v$ remains practically unaltered. However, as pointed out by the reviewer,  $c_v$ is related to the variance of the total energy, and not to the center of its distribution, which is being affected by discretization errors. To emphasize this point, we have rewritten the paragraph avoiding the analogy with the physical phenomena. We have also employed the expression “are not noticeably influenced” in place of “does not get affected”, in order to acknowledge the statistical nature of these measurements.
...
\textbf{Answer}:
We were referring to the improvement in accuracy for time-step sizes up to 5 fs, so we deleted the sentence ``Having overcome such barrier'' and modified that paragraph to emphasize our contribution in that terms.
...
\textbf{Answer}:
 We sincerely thank the reviewer for the kind comments and suggestions. They have all been considered and are answered below.
...
\textbf{Answer}: We thank the reviewer for pointing out this reference, which has been included on page 1 of the manuscript. 
...
\textbf{Answer}:
The reviewer is totally right. We have applied the numerical tests introduced in the suggested reference and verified that the sampled energies are in accordance with the canonical distribution. We have included this information on page 6 of the manuscript.
For time-step sizes with considerable discretization errors, however, those numerical tests are not suitable for analyzing the potential and kinetic energies, since they assume that the Hamiltonian is separable into exclusively momentum- and position-dependent parts, which is no longer true. Thus, we applied the maximum likelihood approach on the total energies only. The results for the NVT-MD methods are reported in the table below. Note that the deviations from the true slope are generally 1$\sigma$ or less for time-step sizes up to 5 fs. This is intimately related to the effect observed when examining the constant-volume heat capacity, where the variance in total energy is not influenced by discretization errors as much as the mean of the distribution. We consider that this result is analogous to the ensemble validation test and, that is why we decided to not include the table below in the revised manuscript.
